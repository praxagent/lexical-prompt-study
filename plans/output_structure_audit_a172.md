# A172: fixed post hoc structure audit of A171 outputs

This is a new, explicitly post hoc, zero-model-call audit. A171's independently
verified aggregate has 96 completed generations, 92 cap failures and four strict
successes. That aggregate motivates this audit; the A172 definitions below must
be frozen before inspecting A171's raw outputs. No A172 finding is asserted here.
The question is which of three prespecified surface structures occur in the
saved outputs, including the cap failures. This is neither a new generation
experiment nor a confirmatory endpoint or rescue score for A171.

## Fixed cohort and evidence boundary

Audit all 96 planned, independently verified, completed outputs from A171
`run-001`. Do not choose outputs by content, outcome, world, placement or wording.
The parent must have a successful bound independent verification, 96 attempted
and completed generations, and no infrastructure failures, interrupted attempts
or missing cells. Require the complete unique 96-result hash manifest from that
...
independent verification, aggregate, records and all 96 individual result SHA-256
...
bytes. Verify all 96 result SHA-256 bindings before parsing or interpreting any
...
leave 96 attempted raw reads but fewer than 96 processed outputs; these counts
...
and scoring were already independently verified for A171. No tokenizer or model
load is needed. Use the frozen A171 plan for its oracle values, active selector,
context and wording; use the original verified `capped` indicator for strata and
...
not infer a replacement cap, correctness or error category from A172 features.

No original sealed held-out input or original A169 context is an A172 input.
Parent evidence is read-only. A171's original strict scores, categories,
...
token belongs to the frozen A171 EOS list, remove that one final token. Remove
...
- All 96 outputs together.
- The six `context` by `wording` groups, each containing 16 outputs.
- Original `cap` and `noncap` strata, containing 92 and four outputs respectively.
- The original cap/noncap strata within each of the six context/wording groups.
...
with the planned 96-output cohort retained and null feature records for
unprocessed outputs. All requested group summaries are null unless the entire
96-output audit is complete; coverage remains explicit. Do not publish
...
why generation failed. The four original noncap responses are descriptive
comparators, not an independently sampled control cohort.
...
population claim, detector search or confirmatory reinterpretation of A171.
...
first raw A171 result read for the audit. Use one fresh audit process, a private
absolute output directory under
`/data2/PRAX/lexical-prompt-study-data/runs/a172/`, an exclusive lock, an immutable
...
API, GPU, download or rental is used. Stop after the fixed 96 outputs, a binding
...
cap strata. Test parent binding drift, exact 96-result coverage, one-shot and
lock behavior, terminal interruption, evidence tampering and stdout/privacy
controls using public synthetic evidence only. These tests inspect no raw A171

"""A191 outer qualification: stdlib/fake workers only, no native imports."""
...
SPEC = importlib.util.spec_from_file_location(
    "a191_outer_test", ROOT / "scripts/run_answer_availability.py"
)
...
def config():
    return {
        "schema_version": "a191-cpu-fp32-config-v1",
...
def test_complete_fake_worker_and_terminal_replay(worker_case):
    ...
    assert terminal["body_entry_counts"] == {"causal_lm_entries": 18, "nested_decoder_entries": 18}
    assert terminal["body_stage_counts"]["technical"] == {
        "causal_lm_entries": 2,
        "nested_decoder_entries": 2,
    }
    assert terminal["body_stage_counts"]["science"] == {
        "causal_lm_entries": 16,
        "nested_decoder_entries": 16,
    }
    assert seen["stages"] == ["technical", "science"] and seen["replays"] == 1
...
def test_post_acquisition_failure_never_publishes_terminal(worker_case, change):
    ...
    if change == "replay":
        ...
        acquired["summary"]["scientific_entries"] = True
    else:
        calls = []
...
def test_completed_terminal_tamper_rejected(worker_case, mutation):
    ...
    if mutation == "bool_count":
        ...
        terminal["body_stage_counts"]["technical"]["causal_lm_entries"] = 3
...
def test_profile_ceiling_stage_order_and_exception_counts(tmp_path):
    ...
    model = LM()
    with profile:
        profile.set_stage("technical")
        model.forward()
        model.forward()
        with pytest.raises(ValueError, match="body_ceiling"):
            model.forward()
    assert profile.counts["causal_lm_entries"] == 3
    assert profile.counts["nested_decoder_entries"] == 2
    assert (tmp_path / "entries/causal_lm_entries_0003.json").exists()
    with pytest.raises(ValueError, match="technical_body_count"):
...
def frozen_case(tmp_path, monkeypatch):
    ...
    runner = source / "scripts/run_answer_availability.py"
    runner.parent.mkdir()
    ...
    relative = "scripts/run_answer_availability.py"
    ...
    monkeypatch.setattr(r, "helper", lambda: Lifecycle())
    frozen = {
        "schema_version": "a191-native-freeze-v1",

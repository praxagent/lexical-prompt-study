...
from lexical_prompt_study.llm_audit_openai_analysis import (
    BINDING_FIELDS, FIXTURE_SHA256, LEGACY_SHA256, load_run, main, summarize_runs,
)
...
def run(tmp_path, index=0, *, selected=2, judgments=("yes", "no")):
    from lexical_prompt_study import llm_audit_openai
    freeze = {
        "schema_version": "a161-openai-freeze-v1", "scope": "synthetic", "run_id": f"synthetic-{index}",
        "protocol_sha256": "a" * 64,
        "source_sha256": hashlib.sha256(Path(llm_audit_openai.__file__).read_bytes()).hexdigest(),
        "packet_sha256": "c" * 64,
        "legacy_source_sha256": LEGACY_SHA256, "fixture_sha256": FIXTURE_SHA256,
        "qualification_sha256": None, "qualification_path": None, "execution_authorized": True,
        "config": {"provider": "openai", "model_name": "gpt-4.1-2025-04-14",
        ...
        "output_root": str(tmp_path / f"run-{index}"), "budget_root": str(tmp_path / "shared-budget"),
        "budget_micro_usd": 10_000_000, "input_token_cap": 1_000_000,
...
def write_store(tmp_path, data):
    ...
    (root / "manifest.json").write_bytes(canonical({"schema_version": "a161-openai-store-v1",
...
def test_provider_explicit_complete_summary_and_runner_schema_agree(tmp_path):
    from lexical_prompt_study.llm_audit_openai import validate_freeze, validate_receipt
    data = run(tmp_path)
    ...
    assert result["overall"]["coverage_by_judge"][0]["valid"] == 2
    assert result["judges"][0]["billing"]["metered_micro_usd"] == 400
...
def test_pair_uncertainty_and_alias_denominators(tmp_path):
    first, second = run(tmp_path), run(tmp_path, 1, judgments=("yes", "uncertain"))
    result = summarize_runs([first, second], mapping(first))
    overall = result["overall"]
    assert overall["unique_prefixes"] == 2
    assert overall["mapped_descriptions"]["horizon_bindings"] == 8
    assert overall["mapped_descriptions"]["repeated_prefix_aliases"] == 6
    counts = overall["all_judges_concordance"]["fields"]["disallowed_assistance"]
    assert counts["all_same"]["yes"] == 1
    assert counts["disagree"] == counts["any_uncertain"] == 1
...
def test_failed_outputs_are_terminal_and_unknown_usage_is_reserved(tmp_path, status, error):
    data = run(tmp_path, selected=1, judgments=("yes",))
...
def test_separate_utility_input_integrates_after_validation(tmp_path):
    ...
    utility = {"schema_version": "a161-utility-mapping-v1", "packet_sha256": numeric["packet_sha256"],
               ...
               "rows": [{"prefix_id": p, "utility_exact_match": None, "refusal_detected": False}
                        for p in data["freeze"]["selection"]]}
    result = summarize_runs([data], numeric, utility)
    comparison = result["semantic_vs_mechanical_utility"]
    assert comparison["per_judge"][0]["distinct_prefixes"]["valid"] == 2
    assert comparison["per_judge"][0]["128"]["expected"] == 2
...
def test_cli_private_immutable_numeric_output(tmp_path, capsys):
    ...
    spec = {"schema_version": "a161-openai-analysis-inputs-v1", "runs": [
    ...
    assert stdout["provider"] == "openai" and stdout["selected_prefixes"] == 2
    assert stat.S_IMODE(out.stat().st_mode) == 0o600
    assert main(args) == 1
    assert json.loads(capsys.readouterr().out) == {"status": "a161_openai_analysis_rejected"}

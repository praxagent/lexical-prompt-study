"""A192 outer qualification: stdlib/fake workers only, no native imports."""
...
SPEC = importlib.util.spec_from_file_location(
    "a192_outer_test", ROOT / "scripts/run_answer_path_ranking.py"
)
...
def config():
    return {
        "schema_version": "a192-cpu-fp32-config-v1",
...
def frozen_case(tmp_path, monkeypatch):
    ...
    runner = source / "scripts/run_answer_path_ranking.py"
    runner.parent.mkdir()
    ...
    relative = "scripts/run_answer_path_ranking.py"
    ...
    monkeypatch.setattr(r, "helper", lambda: Lifecycle())
    frozen = {
        "schema_version": "a192-native-freeze-v1",

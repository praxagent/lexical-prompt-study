"""Invented A192 independent-auditor fixtures; no producer or runtime imports."""
...
from lexical_prompt_study import answer_path_ranking_independent_audit as a
...
class ConstructionTests(unittest.TestCase):
    def test_roster_full_factorial_hash_order_and_balance(self):
        ...
        expected = sorted(
            (
                hashlib.sha256(f"a192-answer-path-ranking-v1|schedule|{core}".encode()).hexdigest(),
                core,
            )
            for core in range(8)
        )
        ...
        for row in a.reconstruct_roster():
            for item, (left, right) in enumerate(row["operands"]):
                for side, actual in enumerate((left, right)):
                    raw = hashlib.sha256(
                        f"a192-answer-path-ranking-v1|{row['core_index']}|{item}|{side}".encode()
                    ).digest()
                    self.assertEqual(

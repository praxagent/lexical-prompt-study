"""A172 public synthetic tests; never inspect parent experimental responses."""
...
from lexical_prompt_study import output_structure_audit as a
...
def fixture(tmp_path, monkeypatch):
    """Wholly synthetic parent and freeze; no real A171 artifacts are opened."""
    ...
    trials = []
    for world, context, wording, selector in itertools.product(range(8), a.CONTEXTS, a.WORDINGS, ("A", "B")):
        index = len(trials)
        trials.append({"trial_id": f"{index:024x}", "sequence_index": index, "world_index": world,
                       "context": context, "wording": wording, "selector": selector,
                       "selected_answer": "public_v1" if selector == "A" else "public_v2",
                       "unselected_answer": "public_v2" if selector == "A" else "public_v1"})
    parent_plan = {"schema_version": "a171-plan-v1", "trials": trials}
    ...
    header = {"schema_version": "a171-run-v1", "plan_sha256": a.file_sha(parent / "inputs/plan.json"),
    ...
    for trial in trials:
        ...
        noncap = (trial["context"], trial["wording"], trial["selector"]) == ("before", "standard", "A") and trial["world_index"] < 4
        ...
        score = {"category": "correct" if noncap else "cap", "strict_correct": noncap, "selected_label": False,
                 "format_valid": noncap, "eos_valid": noncap, "capped": not noncap, "attempted": True,
                 "reason": None if noncap else "token_limit_without_terminal_eos", "response_text": text,
                 "generated_token_count": len(tokens)}
        record = {"trial_id": trial["trial_id"], "sequence_index": index, "context": trial["context"],
                  "wording": trial["wording"], "selector": trial["selector"],
                  **{key: value for key, value in score.items() if key != "response_text"}}
        records.append(record)
        attempt = {"schema_version": "a171-attempt-v1", "run_sha256": a.object_sha(header),
                   "trial_id": trial["trial_id"], "trial_sha256": a.object_sha(trial)}
        result = {"schema_version": "a171-result-v1", "status": "completed", "error": None,
    ...
    coverage = {"attempted": 96, "completed": 96, "infrastructure_failed": 0,
    ...
    write_json(parent / "execution/execution-finished.json", {"schema_version": "a171-execution-finished-v1",
        "status": "finished_schedule", "run_sha256": a.object_sha(header), "interruption": None})
    verifier_path = parent / "frozen/verification/independent_a171.py"
    ...
    parent_freeze = {"source_files_sha256": {"verification/independent_a171.py": a.file_sha(verifier_path)},
    ...
    verification = {"schema_version": "a171-independent-verification-v1", "status": "pass", "coverage": coverage,
                    "terminal_status": "finished_schedule", "independent_native_constructions": 96,
                    "independent_completed_response_scores": 96, "independent_all_planned_rows_and_contrasts_replayed": True,
    ...
    frozen_files = {
        "src/lexical_prompt_study/output_structure_audit.py": Path(a.__file__),
        "plans/output_structure_audit_a172.md": repository / "plans/output_structure_audit_a172.md",
        "tests/test_output_structure_audit.py": Path(__file__),
    }
    for relative, source in frozen_files.items():
        target = root / "frozen" / relative
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_bytes(source.read_bytes())
    protocol = root / "frozen/plans/output_structure_audit_a172.md"
    ...
    freeze = {"schema_version": "a172-freeze-v1", "source_commit": "a" * 40, "pushed_commit": "a" * 40,
...
def numeric_records(fixture):
    ...
    for trial, parent_record in zip(fixture["trials"], fixture["parent_records"], strict=True):
        ...
        result.append({"trial_id": trial["trial_id"], "sequence_index": trial["sequence_index"],
                       "context": trial["context"], "wording": trial["wording"],
...
def test_summaries_match_independent_counts_groups_and_empty_strata(fixture):
    ...
    assert summary["by_cap_status"]["cap"]["outputs"] == 92 and summary["by_cap_status"]["noncap"]["outputs"] == 4
    for context, wording in itertools.product(a.CONTEXTS, a.WORDINGS):
        subset = [row for row in records if (row["context"], row["wording"]) == (context, wording)]
        group = summary["by_context_wording"][context][wording]
        assert group["all"] == independent_group(subset) and group["all"]["outputs"] == 16
    ...
    assert summary["by_context_wording"]["none"]["standard"]["by_cap_status"]["noncap"] == independent_group([])
    overall = summary["overall"]
    assert sum(overall["overlaps"].values()) == 96
...
def test_complete_audit_replay_and_parent_evidence_immutability(fixture):
    ...
    assert aggregate["coverage"] == {"attempted": 96, "processed": 96, "unattempted": 0, "missing": 0}
...
def test_all_96_parent_hashes_and_attempt_receipts_precede_interpretation(fixture, monkeypatch):
    ...
    def parse(raw, trial, record, header):
        assert len(checked) == 96, "text/token interpretation began before every result hash passed"
...
def test_result_hash_failure_stops_before_any_interpretation(fixture, monkeypatch):
    ...
    assert aggregate["coverage"] == {"attempted": 6, "processed": 0, "unattempted": 90, "missing": 96}
    records = json.loads((fixture["args"]["output_root"] / "records.json").read_text())
    assert len(records) == 96 and all(row["features"] is None for row in records)
...
def test_parent_metadata_hash_drift_blocks_all_raw_output_reads(fixture, monkeypatch, relative):
    ...
    aggregate = a.run_plan(**fixture["args"])
    assert aggregate["coverage"] == {"attempted": 0, "processed": 0, "unattempted": 96, "missing": 96}
    assert aggregate["execution_status"] == "failed" and aggregate["summaries"] is None
...
def test_interruption_preserves_two_phase_coverage_and_replays_completed_only(fixture, monkeypatch, phase, number):
    ...
    aggregate = a.run_plan(**fixture["args"])
    expected_processed, expected_attempted = (0, 4) if phase == "hash" else (3, 96)
    ...
    assert aggregate["coverage"] == {"attempted": expected_attempted, "processed": expected_processed,
                                     "unattempted": 96 - expected_attempted, "missing": 96 - expected_processed}
    ...
    assert receipt["interruption"] == {
        ...
        "A172Deadline" if number == signal.SIGALRM else "A172Interrupted"}
    ...
    assert sum(row["features"] is None for row in replay["records"]) == 96 - expected_processed
...
def test_signal_after_durable_result_publication_keeps_that_result(fixture, monkeypatch):
    ...
    aggregate = a.run_plan(**fixture["args"])
    assert aggregate["coverage"] == {"attempted": 96, "processed": 2, "unattempted": 0, "missing": 94}
    ...
    if tamper == "orphan":
        ...
        if tamper == "feature":
            ...
            value["interruption"] = {"signal_number": 15, "signal_name": "SIGTERM", "pid": os.getpid(),
                                     "exception_type": "A172Interrupted"}
...
def test_cli_returns_only_status_coverage_and_no_private_exception_text(fixture, monkeypatch, capsys):
    ...
    assert json.loads(captured.out) == {"status": "a172_failed", "error_type": "RuntimeError", "target_model_calls": 0}
...
def test_audit_freeze_and_review_gates_precede_parent_response_reads(fixture, monkeypatch, change):
    ...
    if change in ("source", "protocol", "tests"):
        relative = {"source": "src/lexical_prompt_study/output_structure_audit.py",
                    "protocol": "plans/output_structure_audit_a172.md", "tests": "tests/test_output_structure_audit.py"}[change]
...
def test_malformed_detector_schema_is_terminal_before_numeric_publication(fixture, monkeypatch):
    ...
    def invalid(*args):
        ...
        assert result["answer_object_count"] == 0
        result["answer_object_count"] = False
        return result

    monkeypatch.setattr(a, "inspect_output", invalid)
    aggregate = a.run_plan(**fixture["args"])
    assert len(calls) == 1 and aggregate["execution_status"] == "failed"
    assert aggregate["coverage"] == {"attempted": 96, "processed": 0, "unattempted": 0, "missing": 96}
    assert aggregate["summaries"] is None

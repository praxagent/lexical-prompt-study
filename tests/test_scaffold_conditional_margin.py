"""A180 public synthetic qualification; no target tokenizer or pretrained weights."""
...
from lexical_prompt_study import scaffold_conditional_margin as a
...
MATERIALS = {'full': 'Public full material.', 'sham': 'Public sham fixture with a distinct length.',
             'inert': 'Public inert words for qualification only.'}
...
class Tokenizer:
    ...
    def get_chat_template(self):
        return 'a180 public synthetic native template'
...
def fixture(tmp_path, monkeypatch):
    ...
    protocol.write_text("Public synthetic A180 protocol fixture")
...
def test_exact_three_protocol_templates_and_original_teacher_force_alias():
    import re
    text = Path('plans/scaffold_conditional_margin_a180.md').read_text()
...
def test_native_paths_share_exact_fixed_three_tokens_not_longest_prefix(fixture):
    ...
    assert native['planned_contexts'] == 56 and native['planned_forward_calls'] == 224
    ...
    class BadTokenizer(Tokenizer):
        ...
        def encode(self, text, **kwargs):
            ...
            if change == 'boundary' and text.endswith('"}') and JSON_PREFIX in text:
                result[0] += 1
            if change == 'length' and '"answer":"s101"' in text:
                result.append(30)
            return result
...
def test_complete_schedule_replays_all_224_fresh_measurements_privately(fixture):
    ...
    assert aggregate['coverage'] == {'attempted': 224, 'completed': 224, 'infrastructure_failed': 0,
                                     'interrupted': 0, 'unattempted': 0, 'missing': 0}
    assert aggregate['complete_contexts'] == 56 and aggregate['numerical_valid'] == {'true': 56, 'false': 0, 'unknown': 0}
...
def test_numerical_or_functional_failure_never_gates_or_retries(fixture, mode):
    ...
    assert len(calls) == len(set(calls)) == 224 and aggregate['status'] == 'complete'
...
def test_infrastructure_failure_is_unknown_and_all_remaining_slots_are_attempted(fixture):
    ...
    assert len(calls) == 224 and aggregate['status'] == 'incomplete' and aggregate['clean_qualification']['qualified'] is None
    assert aggregate['coverage'] == {'attempted': 224, 'completed': 223, 'infrastructure_failed': 1,
...
def test_invalid_forward_measurement_is_infrastructure_not_a_repaired_score(fixture, change):
    ...
    assert len(calls) == 224 and aggregate['coverage']['infrastructure_failed'] == 1 and aggregate['clean_qualification']['qualified'] is None
...
def test_export_rejects_tampered_bindings_arithmetic_and_coverage(fixture, tamper):
    ...
    if tamper == 'orphan':
        ...
        if tamper == 'native':
            ...
            value['interruption'] = {'signal_number': 15, 'signal_name': 'SIGTERM', 'pid': os.getpid(), 'exception_type': 'A180Interrupted'}
        elif tamper == 'records':
...
def test_loader_failure_consumes_one_shot_and_cannot_change_output_path(fixture, monkeypatch):
    ...
    assert aggregate["coverage"]["unattempted"] == aggregate["coverage"]["missing"] == 224
...
def test_system_exit_is_not_a_recoverable_evaluation_error(fixture):
    ...
    assert a.export_run(**fixture[1])["aggregate"]["coverage"]["unattempted"] == 224
...
def test_preflight_interrupt_preserves_startup_receipt_and_consumes_claim(fixture, monkeypatch, number):
    ...
    exception = KeyboardInterrupt if number is None else a.A180Deadline if number == signal.SIGALRM else a.A180Interrupted
...
def test_cli_exposes_only_safe_status_coverage_and_private_log(fixture, monkeypatch, capsys):
    def fake_run(**kwargs):
        ...
        return {"status": "complete", "execution_status": "finished_schedule",
                "coverage": {"attempted": 224, "completed": 224}, "response_text": "synthetic private response"}
...
def test_signal_after_result_publication_preserves_durable_completed_count(fixture, monkeypatch):
    ...
    assert aggregate['coverage'] == {'attempted': 1, 'completed': 1, 'infrastructure_failed': 0,
                                     'interrupted': 0, 'unattempted': 223, 'missing': 223}
...
def test_runtime_signal_and_deadline_preserve_attempts_and_null_margins(fixture, number):
    ...
    assert aggregate['coverage'] == {'attempted': 4, 'completed': 3, 'infrastructure_failed': 0,
                                     'interrupted': 1, 'unattempted': 220, 'missing': 221}
    ...
    assert receipt['interruption'] == {
        ...
        'A180Deadline' if number == signal.SIGALRM else 'A180Interrupted'}
    replay = a.export_run(**fixture[1])
    assert replay['aggregate'] == aggregate and len(replay['records']) == 56
...
def test_prefix_ids_must_be_identical_across_all_fifty_six_contexts(fixture):
...
def test_exact_fresh_worlds_seven_conditions_and_224_forward_schedule(fixture):
    plan = fixture[0]
    contexts, evaluations = plan['contexts'], plan['evaluations']
    assert tuple(a.VALUE_PAIRS) == PAIRS
    assert len(contexts) == len({row['context_id'] for row in contexts}) == 56
    assert len(evaluations) == len({row['evaluation_id'] for row in evaluations}) == 224
    assert [row['sequence_index'] for row in evaluations] == list(range(224))
    assert [row['context_index'] for row in contexts] == list(range(56))
    assert plan['planned_value_pairs'] == 2 and plan['planned_world_assignments'] == 4
    assert plan['planned_contexts'] == 56 and plan['planned_forward_calls'] == 224
    orders = []
    for base in range(8):
        world, selector_index = divmod(base, 2)
        selector = SELECTOR_ORDERS[world][selector_index]
        values = PAIRS[world // 2][::-1] if world % 2 else PAIRS[world // 2]
        mapping = dict(zip(('A', 'B'), values, strict=True))
        compact = json.dumps(mapping, separators=(',', ':'))
        rotated = CONDITIONS[2 * world:] + CONDITIONS[:2 * world]
        expected_order = rotated[::-1] if selector_index else rotated
        cells = contexts[base * 7:(base + 1) * 7]
        assert tuple(row['condition'] for row in cells) == expected_order
        orders.append(expected_order)
        for row in cells:
            assert row['base_context_index'] == base and row['selector'] == selector
            ...
            if row['condition'] == 'clean':
                kind, placement, user = 'none', 'none', compact
            else:
                kind, placement = row['condition'].split('_')
                user = MATERIALS[kind] + '\n\n' + compact if placement == 'before' else compact + '\n\n' + MATERIALS[kind]
            assert row['scaffold_kind'] == kind and row['placement'] == placement
            assert row['messages'] == [
                {'role': 'system', 'content': ' '.join((SELECTION, FORMAT, CLOSING)).format(selector=selector)},
                {'role': 'user', 'content': user}]
            assert row['selected_answer'] == mapping[selector]
            assert row['selected_candidate'] == ('value_1' if mapping[selector] == PAIRS[world // 2][0] else 'value_2')
            assert row['other_candidate'] != row['selected_candidate']
            assert row['canonical_candidates'] == {
                f'value_{i+1}': json.dumps({'answer': value}, separators=(',', ':'))
                for i, value in enumerate(PAIRS[world // 2])}
    ...
    for left, right in itertools.combinations(CONDITIONS, 2):
        assert sum(order.index(left) < order.index(right) for order in orders) == 4
    assert Counter(row['selector'] for row in contexts) == {'A': 28, 'B': 28}
    assert Counter(row['condition'] for row in contexts) == dict.fromkeys(CONDITIONS, 8)
    assert plan['materials'] == MATERIALS
    assert all(plan[key] is False for key in ('padding_allowed', 'resume_allowed', 'heldout_allowed',
                                             'interim_gate', 'generation_allowed', 'warmup_allowed'))
...
def test_material_provenance_and_condition_identity_tampering_is_rejected(fixture, change):
    ...
    if change == 'plan_material':
        ...
        row[key] = {'condition': 'clean', 'placement': 'after', 'base_context': 1}[change]
    with pytest.raises(ValueError):
        a.validate_plan(plan)
...
def all_contrasts(aggregate):
    assert set(aggregate['contrasts']['mean_margin_nats']) == {'full_minus_sham_before', 'full_minus_sham_after'}
    assert set(aggregate['secondary_contrasts']['mean_margin_nats']) == {'inert_minus_clean_before', 'inert_minus_clean_after'}
    return {**aggregate['contrasts']['mean_margin_nats'], **aggregate['secondary_contrasts']['mean_margin_nats']}
...
def test_continuous_contrasts_use_paired_equal_weights_and_can_exceed_binary_bounds(fixture):
    from fractions import Fraction
    offsets = {'clean': 8, 'full_before': -11, 'full_after': 13, 'sham_before': 2,
               'sham_after': -3, 'inert_before': -7, 'inert_after': -5}

    def margin(base, condition):
        return offsets[condition] + Fraction(base, 4) * (CONDITIONS.index(condition) + 1)

    records, aggregate = signed_fixture(fixture, margin)
    assert len(records) == 56 and aggregate['clean_qualification']['qualified'] is True
    for name, result in all_contrasts(aggregate).items():
        positive, negative = COMPARISONS[name]
        expected = float(sum(margin(base, positive) - margin(base, negative) for base in range(8)) / 8)
        assert result == {'planned_contexts': 16, 'resolved_contexts': 16, 'planned_pairs': 8, 'resolved_pairs': 8,
                          'point': expected, 'lower': expected, 'upper': expected, 'unbounded': False}
        assert abs(expected) > 1
    assert aggregate['contrasts']['mean_margin_nats']['full_minus_sham_before']['point'] < 0
    assert aggregate['contrasts']['mean_margin_nats']['full_minus_sham_after']['point'] > 0
    for condition in CONDITIONS:
        group = aggregate['by_condition'][condition]
        expected = float(sum(margin(base, condition) for base in range(8)) / 8)
        assert group['point'] == group['lower'] == group['upper'] == expected and group['unbounded'] is False
        assert group['planned_contexts'] == group['resolved_contexts'] == 8
        assert group['complete_contexts'] == 8
        assert sum(group['mean_margin_signs'].values()) == sum(group['worst_margin_signs'].values()) == 8


@pytest.mark.parametrize('condition', CONDITIONS)
@pytest.mark.parametrize('failure', ['missing', 'numeric'])
def test_contrast_missingness_is_local_and_condition_mean_never_drops_unresolved(fixture, condition, failure):
    missing = next(c['context_index'] for c in fixture[0]['contexts']
                   if c['base_context_index'] == 3 and c['condition'] == condition)
    kwargs = {'missing_contexts' if failure == 'missing' else 'numeric_failed_contexts': (missing,)}
    records, aggregate = signed_fixture(fixture, lambda base, cond: 10 + CONDITIONS.index(cond) + base / 8, **kwargs)
    assert records[missing]['mean_margin_nats'] is None and records[missing]['margins_unbounded'] is True
    for name, result in all_contrasts(aggregate).items():
        affected = condition in COMPARISONS[name]
        assert result['planned_contexts'] == 16 and result['planned_pairs'] == 8
        assert result['resolved_contexts'] == (15 if affected else 16)
        assert result['resolved_pairs'] == (7 if affected else 8)
        assert result['unbounded'] is affected
        if affected:
            assert result['point'] is result['lower'] is result['upper'] is None
        else:
            assert result['point'] is not None and result['lower'] == result['point'] == result['upper']
    group = aggregate['by_condition'][condition]
    assert group['planned_contexts'] == 8 and group['resolved_contexts'] == 7
    assert group['point'] is group['lower'] is group['upper'] is None and group['unbounded']
    assert group['mean_margin_signs']['unknown'] == group['worst_margin_signs']['unknown'] == 1
    assert aggregate['clean_qualification']['qualified'] is (
        (None if failure == 'missing' else False) if condition == 'clean' else True)


def test_shared_clean_margin_is_counted_once_and_enters_both_secondaries_equally(fixture):
    _, baseline = signed_fixture(fixture, lambda base, cond: 2.)
    _, changed = signed_fixture(fixture, lambda base, cond: 10. if base == 0 and cond == 'clean' else 2.)
    before, after = all_contrasts(baseline), all_contrasts(changed)
    assert changed['clean_qualification']['planned_contexts'] == 8
    assert changed['by_condition']['clean']['resolved_contexts'] == 8
    for name in COMPARISONS:
        assert after[name]['point'] - before[name]['point'] == (-1. if name.startswith('inert') else 0.)
    assert sum(value['planned_contexts'] for value in changed['by_condition'].values()) == 56


@pytest.mark.parametrize('scaffold_margin', [-9., 0., .0005, .001, 10.])
def test_scaffold_sign_and_small_margin_are_outcomes_without_functional_filter(fixture, scaffold_margin):
    records, aggregate = signed_fixture(fixture, lambda base, cond: 2. if cond == 'clean' else scaffold_margin)
    assert aggregate['clean_qualification']['qualified'] is True
    assert aggregate['numerical_valid'] == {'true': 56, 'false': 0, 'unknown': 0}
    for row in records:
        assert row['numerical_valid'] is True
        if row['condition'] == 'clean':
            assert row['functional_applicable'] is True and row['functional_passed'] is True
        else:
            ...
            assert row['mean_margin_nats'] == row['worst_margin_nats'] == scaffold_margin
            assert row['mean_reversed'] is row['worst_reversed'] is (scaffold_margin < 0)
            assert row['mean_tie'] is row['worst_tie'] is (scaffold_margin == 0)
    assert all(result['point'] is not None for result in all_contrasts(aggregate).values())


@pytest.mark.parametrize('clean_margin', [-2., 0., .0005, .001])
def test_failed_clean_interpretation_gate_does_not_erase_valid_scores_or_contrasts(fixture, clean_margin):
    records, aggregate = signed_fixture(fixture, lambda base, cond: clean_margin if cond == 'clean' else -3.)
    assert aggregate['clean_qualification']['qualified'] is False
    assert all(row['mean_margin_nats'] is not None for row in records)
    assert all(not result['unbounded'] for result in all_contrasts(aggregate).values())
    assert aggregate['secondary_contrasts']['mean_margin_nats']['inert_minus_clean_before']['point'] == -3. - clean_margin


def test_fixed_candidate_preference_fails_half_clean_controls_without_discarding_scaffolds(fixture):
    plan = fixture[0]
    prepared = prepare(fixture)['native_inputs']
    results = {}
    for evaluation in plan['evaluations']:
    ...
    assert sum(row['functional_passed'] for row in clean) == 4
    assert sum(row['mean_reversed'] for row in records) == 28
    assert aggregate['clean_qualification']['qualified'] is False
    assert all(result['point'] == 0. for result in all_contrasts(aggregate).values())


def test_compiled_material_receipts_and_materials_do_not_alias_validation_pins(fixture):
    plan = fixture[0]
    receipts = copy.deepcopy(a.MATERIAL_RECEIPTS)
    original_materials = dict(MATERIALS)
    plan['material_receipts']['full']['bytes'] += 1
    plan['materials']['full'] += ' changed'
    assert a.MATERIAL_RECEIPTS == receipts and MATERIALS == original_materials
    with pytest.raises(ValueError):
        a.validate_plan(plan)


def test_all_224_forwards_continue_with_negative_scaffold_margins(fixture):
    plan = fixture[0]

    def negative_scaffolds(value, index):
        evaluation = plan['evaluations'][index]
        context = plan['contexts'][evaluation['context_index']]
        if context['condition'] == 'clean':
            return value
        selected = evaluation['candidate'] == context['selected_candidate']
        return measurement(len(value['token_logprobs']), -.2 if selected else -.01)

    aggregate, calls = execute(fixture, scorer_transform=negative_scaffolds)
    assert len(calls) == len(set(calls)) == 224 and aggregate['coverage']['completed'] == 224
    assert aggregate['clean_qualification']['qualified'] is True
    records = a.export_run(**fixture[1])['records']
    assert sum(row['mean_margin_nats'] < 0 for row in records) == 48
    assert all(row['functional_passed'] is None for row in records if row['condition'] != 'clean')
    assert all(not result['unbounded'] for result in all_contrasts(aggregate).values())


@pytest.mark.parametrize('other_base,expected_pairs', [(0, 7), (1, 6)])
def test_paired_coverage_distinguishes_two_missing_cells_in_same_or_different_pair(fixture, other_base, expected_pairs):
    missing = [context['context_index'] for context in fixture[0]['contexts']
               if (context['base_context_index'], context['condition']) in
               ((0, 'full_before'), (other_base, 'sham_before'))]
    _, aggregate = signed_fixture(fixture, lambda base, condition: 3., missing_contexts=missing)
    result = aggregate['contrasts']['mean_margin_nats']['full_minus_sham_before']
    assert result['resolved_contexts'] == 14 and result['resolved_pairs'] == expected_pairs
    assert result['point'] is result['lower'] is result['upper'] is None and result['unbounded'] is True
    assert aggregate['contrasts']['mean_margin_nats']['full_minus_sham_after']['point'] == 0.

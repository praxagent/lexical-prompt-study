"""A197 outer qualification: stdlib/fake workers only, no native imports."""
...
SPEC = importlib.util.spec_from_file_location(
    "a197_outer_test", ROOT / "scripts/run_rule_cue_interaction.py"
)
...
def config():
    return {
        "schema_version": "a197-cpu-fp32-config-v1",
...
def test_complete_fake_worker_and_terminal_replay(worker_case):
    ...
    assert terminal["body_entry_counts"] == {
        "causal_lm_entries": 128,
        "nested_decoder_entries": 128,
    }
...
def test_completed_terminal_tamper_rejected(worker_case, mutation):
    ...
    if mutation == "bool_count":
        ...
        terminal["body_stage_counts"]["science"]["causal_lm_entries"] = 127
...
def test_profile_ceiling_stage_order_and_exception_counts(tmp_path):
    ...
    with profile:
        ...
        with pytest.raises(ValueError, match="body_ceiling"):
            model.forward()
    assert profile.counts["causal_lm_entries"] == 129
    assert profile.counts["nested_decoder_entries"] == 128
    assert (tmp_path / "entries/causal_lm_entries_0129.json").exists()
...
def frozen_case(tmp_path, monkeypatch):
    ...
    runner = source / "scripts/run_rule_cue_interaction.py"
    runner.parent.mkdir()
    ...
    predecessor_verification = raw_json(
        prepared / "A196-VERIFICATION.json", {"invented_verified": True}
    )
    predecessor_closure = raw_json(prepared / "A196-CLOSED.json", {"invented_closed": True})
    monkeypatch.setattr(r, "PREDECESSOR_VERIFICATION_SHA256", predecessor_verification)
    monkeypatch.setattr(r, "PREDECESSOR_CLOSURE_SHA256", predecessor_closure)
    relative = "scripts/run_rule_cue_interaction.py"
    ...
    monkeypatch.setattr(r, "helper", lambda: Lifecycle())
    frozen = {
        "schema_version": "a197-native-freeze-v1",
        ...
        "predecessor_verification_sha256": predecessor_verification,
        "predecessor_closure_sha256": predecessor_closure,
...
def test_all_inherited_and_special_inventory_pins_match_actual_source():
    ...
    assert all(
        "cue_validity_ranking" not in path and "canonical_answer_ranking" not in path
        for path in r.SOURCE_PATHS
    )


def test_public_runtime_keeps_exact_selected_cpu_policy_and_128_scope():
    value = json.loads((ROOT / "plans/rule_cue_interaction_a197_runtime.public.json").read_bytes())
    r.validate_config(value)
    assert value["execution"]["planned_forwards"] == 128
...
def test_actual_new_core_preparation_metadata_api_without_native_calls(monkeypatch):
    ...
    core = importlib.import_module("lexical_prompt_study.rule_cue_interaction_acquisition")
...
@pytest.mark.parametrize(
    "key,name",
    [
        ("predecessor_verification_sha256", "A196-VERIFICATION.json"),
        ("predecessor_closure_sha256", "A196-CLOSED.json"),
    ],
)
@pytest.mark.parametrize(
    "tamper", ["different_copied_artifact", "wrong_named_artifact", "bool_pin", "missing_field"]
)
def test_predecessor_provenance_exact_copied_binding(frozen_case, key, name, tamper):
    frozen, path, _ = frozen_case
    if tamper == "different_copied_artifact":
        other = raw_json(path.parent / "unrelated.json", {"different": True})
        frozen["files_sha256"]["unrelated.json"] = other
        frozen[key] = other
    elif tamper == "wrong_named_artifact":
        original = path.parent / name
        moved = path.parent / ("misnamed-" + name)
        original.rename(moved)
        wanted = frozen["files_sha256"].pop(name)
        frozen["files_sha256"][moved.name] = wanted
    elif tamper == "bool_pin":
        frozen[key] = True
    else:
        del frozen[key]
    sha = raw_json(path, frozen)
...
def test_expanded_scope_changes_only_planned_count_in_execution_policy():
    # Read prior immutable source as AST only; never import an old study runner.
    import ast

    prior = ast.parse((ROOT / "scripts/run_cue_validity_ranking.py").read_text())
    current = ast.parse((ROOT / "scripts/run_rule_cue_interaction.py").read_text())
    ...
    for name in (
        ...
        "LIMITS",
        "ENV",
        "INSTRUMENT_QUALIFICATION_SHA256",
    ):
        assert assignment(prior, name) == assignment(current, name)
    ...
    def execution(tree):
        return next(
            ...
            if isinstance(node, ast.Assign)
            and any(
                isinstance(target, ast.Name) and target.id == "EXECUTION" for target in node.targets
            )
        )

    old_policy, new_policy = execution(prior), execution(current)
    assert old_policy.pop("planned_forwards") == 64
    assert new_policy.pop("planned_forwards") == 128
    assert old_policy == new_policy
    assert r.EXECUTION["planned_forwards"] == 128
    assert set(r.SOURCE_PATHS) - set(r.PINS) == {
...
def test_old_64_measurement_summary_cannot_complete_new_run(worker_case):
    frozen, core, _, _, acquired = worker_case
    for key in (
        ...
        acquired["summary"][key] = 64
    with pytest.raises(ValueError):
        r.run_worker(frozen)
    root = Path(frozen["output_root"]) / "worker"
    assert not (root / "worker-terminal.json").exists()
    assert r.read_json(root / "worker-failure.json")["body_entry_counts"] == {
        "causal_lm_entries": 128,
        "nested_decoder_entries": 128,
    }


def test_half_body_count_cannot_complete_claimed_128_summary(worker_case):
    frozen, core, _, _, acquired = worker_case

    def half(model, tokenizer, *, plan, prepared, runtime_binding, directory, event):
        Path(directory).mkdir()
        r.publish(Path(directory) / "terminal.json", {"invented": True})
        event("science")
        for _ in range(64):

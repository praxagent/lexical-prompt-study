"""A189 invented generation-only qualification; no pretrained/native assets."""
...
from lexical_prompt_study import gpu3b_arithmetic_task_load_acquisition as a
...
class Tokenizer:
    ...
    def get_chat_template(self):
        return "invented-a189-native-template"
    ...
    def apply_chat_template(
        ...
        assert date_string == "23 Sep 2026" and return_dict is False
...
class Model(torch.nn.Module):
    def __init__(self, *, reached=False, wrong=False, cap=False):
        ...
        self.error = None
        self.callback = None
        self.to(device="cuda:0", dtype=torch.bfloat16)
        self.eval()
    ...
    def forward(self, **kwargs):
        ...
        if index == 0 and self.cap:
            ...
            chosen = (
                0 if step < 4 else 254 if self.wrong and step == 4 else 20 if step == 4 else 255
            )
        else:
            chosen = 255
        logits = torch.zeros((1, 1, 256), dtype=torch.bfloat16, device="cuda:0")
        logits[0, 0, chosen] = 1
...
def test_t4_is_checkpoint_not_capture_and_no_future_leak(tmp_path, wrong):
    result, model, plan, prepared = run(tmp_path / "run", Model(reached=True, wrong=wrong))
    assert result["summary"]["total_entries"] == len(model.entries) == 37
    prompt = prepared["observations"][0]["prompt_token_ids"]
    assert [x["input_ids"].tolist()[0] for x in model.entries[:6]] == [
        prompt + [0] * k for k in range(5)
    ] + [prompt + [0] * 4 + [254 if wrong else 20]]
    ...
    for call in model.entries:
        ...
        assert torch.equal(
            call["attention_mask"], torch.ones((1, n), dtype=torch.int64, device="cuda:0")
        )
        assert torch.equal(call["position_ids"], torch.arange(n, device="cuda:0").reshape(1, n))
    clean(model)
...
def test_interruption_after_t4_keeps_only_available_diagnostic(tmp_path):
    ...
    assert first["unflagged_remaining_ge2"] is True and result["summary"]["total_entries"] == 5
...
def test_tiny_untrained_llama32_cachefree_body_entries(tmp_path):
    ...
    model = LlamaForCausalLM(config).to(device="cuda:0", dtype=torch.bfloat16).eval()
    with torch.no_grad():
...
def test_t4_decode_failure_preserves_reached_with_unknown_diagnostic(tmp_path):
    model, tok, plan, prepared, runtime = setup(Model(reached=True))
    decode = tok.decode
...
def test_fresh_bf16_plan_binds_native_dtype_technical_messages_and_separate_budgets():
    model, tok, plan, prepared, runtime = setup()
    assert all(p.dtype == torch.bfloat16 for p in model.parameters())
    assert plan["generation_policy"]["native_logits_dtype"] == "bfloat16"
    assert plan["generation_policy"]["retained_logits"] == "native_bf16_and_exact_fp32"
    assert plan["max_entries"] == 2048
    assert plan["diagnostic"]["non_eos_prefix_tokens"] == 4
    assert plan["chat_template_date"] == a.tasks.CHAT_TEMPLATE_DATE == a.DATE
    assert plan["generation_policy"]["device"] == "cuda:0"
    assert plan["generation_policy"]["sdpa_backend"] == "math"
    assert plan["technical_check"]["maximum_total_entries"] == 2050
    assert plan["technical_check"]["forwards"] == 2
    assert plan["technical_check"]["messages"] == a.tasks.technical_messages()
    changed = a.e._snapshot(plan)
    changed["technical_check"]["forwards"] = 3
    with pytest.raises(ValueError):
        a.validate_plan(changed)
    row = plan["rows"][0]
    call = a._call_attempt(plan, "0" * 64, row, 0, [1, 2])
    assert call["schema_version"] == a.primitive.SCHEMA


def test_wrong_parameter_precision_rejected_before_acquisition_claim(tmp_path):
    model, tok, plan, prepared, runtime = setup()
    model.float()
    with pytest.raises(ValueError):
        invoke(tmp_path / "run", model, tok, plan, prepared, runtime)
...
def test_cuda_sync_failure_after_return_stays_unusable(tmp_path, monkeypatch):
    model, tok, plan, prepared, runtime = setup()
    error = RuntimeError("invented asynchronous CUDA failure")

    def fail_sync(_torch):
        raise error

    monkeypatch.setattr(a.primitive, "_synchronize", fail_sync)
    with pytest.raises(RuntimeError) as caught:
        invoke(tmp_path / "run", model, tok, plan, prepared, runtime)
    assert caught.value is error
    failure = a._read(tmp_path / "run/rows/000/generation/step_00/failure.json")
    assert failure["model_call_returned"] is True
    assert failure["cuda_synchronized"] is False and failure["usable"] is False
    replayed = a.load_acquisition(tmp_path / "run", plan)
    assert replayed["summary"]["total_entries"] == 1
    assert all(row["label"] is None for row in replayed["records"])
    clean(model)

"""Invented A196 native-text, arithmetic and durable-receipt controls; no real model."""
...
class CoreTests(unittest.TestCase):
    ...
    def test_both_cues_share_candidate_ids_but_keep_distinct_prompt_inputs(self):
        grouped = {}
        for row, obs in zip(self.plan["rows"], self.prepared["observations"], strict=True):
            grouped.setdefault(row["core_index"], {})[row["condition"]] = (row, obs)
        self.assertEqual(len(grouped), 8)
        for pair in grouped.values():
            self.assertEqual(set(pair), {"true_cue", "foil_cue"})
            (_, true), (_, foil) = pair["true_cue"], pair["foil_cue"]
            self.assertNotEqual(true["prompt_utf8"], foil["prompt_utf8"])
            self.assertNotEqual(true["prompt_token_ids"], foil["prompt_token_ids"])
            for candidate in ("arithmetic_truth", "arithmetic_foil"):
                self.assertEqual(
                    true["candidates"][candidate]["target_token_ids"],
                    foil["candidates"][candidate]["target_token_ids"],
                )
            self.assertEqual(
                set(true["payload_token_ids"]),
    ...
    def test_repeat_mismatch_retains_all64(self):
        ...
        self.assertIsNone(
            result["analysis"]["contrasts"]["margin_nats"]["true_cue_minus_foil_cue"]["point"]
        )

"""A174 synthetic protocol qualification; no target tokenizer, model or raw parents.

Shared synthetic fixtures exercise inherited guards. All component bytes,
schedule weights, main effects and secondary interaction are specified below.
"""
...
from lexical_prompt_study import closing_constraint_crossover as a


EXPECTED_WORLDS = (("s40", "s47", "AB"), ("s44", "s41", "BA"),
                   ("s43", "s46", "BA"), ("s45", "s42", "AB"))
AXES = ("instruction_core", "closing_constraint")
LEVELS = (("legacy", "a171_standard"), ("legacy", "a171_standard"))
EXPECTED_BASE = tuple(itertools.product(*LEVELS, ("A", "B")))
LEGACY_CORE = ('The authoritative query selector is {selector}. The user supplies two explicit answer values labeled A and B. '
               'Copy the value with the authoritative label. Output exactly one JSON object with the single field "answer" '
               'containing that value as a string.')
STANDARD_CORE = ('Select the entry labeled {selector} from the user\'s mapping. Return that entry\'s mapped value '
                 'as exactly one JSON object with the single key "answer" and a string value.')
LEGACY_CLOSING = 'Do not transform the value or include other fields, explanations, or Markdown fences.'
STANDARD_CLOSING = 'Do not return any other text.'
...
class Tokenizer:
    ...
    def get_chat_template(self):
        return "a174 public synthetic native template"
...
def fixture(tmp_path, monkeypatch):
    ...
    config = {
        ...
        "max_prompt_tokens": 4032, "max_new_tokens": 64, "seed": 1, "cpu_threads": 4, "gpu_memory_gib": 10,
    }
    config_path = tmp_path / "config.json"
    protocol = tmp_path / "protocol.md"
    reference = tmp_path / "reference.py"
    config_path.write_bytes(a.tasks.canonical(config))
    protocol.write_text("Public synthetic A174 protocol fixture")
...
def test_native_preparation_replays_every_public_cell_without_consumption(fixture):
    ...
    assert prepared["planned_cells"] == prepared["maximum_generation_calls"] == 32
...
def test_infrastructure_failure_is_unresolved_and_never_retried(fixture):
    ...
    assert aggregate["overall"]["strict_accuracy"] == {
        "point": None, "lower": 31 / 32, "upper": 1., "planned_outcomes": 32, "resolved_outcomes": 31}
...
def test_signal_deadline_and_keyboard_interrupt_preserve_coverage(fixture, number):
    ...
    assert receipt["interruption"] == {
        ...
        "A174Deadline" if number == signal.SIGALRM else "A174Interrupted"}
...
def test_native_vocabulary_bound_is_not_inferred_from_decoder_acceptance(fixture, monkeypatch):
    ...
    assert len(prepare(exact)["native_inputs"]) == 32
...
def test_preflight_interrupt_preserves_startup_receipt_and_consumes_claim(fixture, monkeypatch, number):
    ...
    exception = KeyboardInterrupt if number is None else a.A174Deadline if number == signal.SIGALRM else a.A174Interrupted
...
def test_exact_components_worlds_and_counterbalanced_schedule(fixture):
    plan, _ = fixture
    trials = plan['trials']
    assert tuple(a.WORLDS) == EXPECTED_WORLDS
    assert tuple(a.BASE_SCHEDULE) == EXPECTED_BASE
    assert a.CORE_TEXT == {'legacy': LEGACY_CORE, 'a171_standard': STANDARD_CORE}
    assert a.CLOSING_TEXT == {'legacy': LEGACY_CLOSING, 'a171_standard': STANDARD_CLOSING}
    assert len(trials) == len({trial['trial_id'] for trial in trials}) == 32
    assert [trial['sequence_index'] for trial in trials] == list(range(32))
    positions = []
    for world, (value_a, value_b, order) in enumerate(EXPECTED_WORLDS):
        ...
        for trial in cells:
            core = LEGACY_CORE if trial['instruction_core'] == 'legacy' else STANDARD_CORE
            closing = LEGACY_CLOSING if trial['closing_constraint'] == 'legacy' else STANDARD_CLOSING
            expected_system = core.format(selector=trial['selector']) + ' ' + closing
            ...
            assert not set(trial).intersection({'context', 'wording', 'mapping_heading', 'value_form'})
        assert len({cell['messages'][1]['content'] for cell in cells}) == 1
    assert len(list(itertools.combinations(EXPECTED_BASE, 2))) == 28
    ...
    assert len(values) == 8 and values.isdisjoint({f's{i:02d}' for i in range(40)})
    ...
    assert plan['secondary_interaction'] == {
        'name': 'core_by_constraint_interaction',
        'positive_cells': [['legacy', 'legacy'], ['a171_standard', 'a171_standard']],
        'negative_cells': [['legacy', 'a171_standard'], ['a171_standard', 'legacy']],
        'planned_quartets': 8, 'cell_coefficient_magnitude': .125}
    assert plan['planned_cells'] == plan['maximum_generation_calls'] == 32 and plan['max_new_tokens'] == 64
...
def test_protocol_components_exact_diagonals_and_identity_aliases():
    import re
    text = Path('plans/closing_constraint_crossover_a174.md').read_text()
    assert re.findall(r'```text\n([^\n]+)\n```', text) == [LEGACY_CORE, STANDARD_CORE, LEGACY_CLOSING, STANDARD_CLOSING]
    for selector in ('A', 'B'):
        assert a.system_instruction('legacy', 'legacy', selector) == a.tasks.system_instruction(selector)
        assert a.system_instruction('a171_standard', 'a171_standard', selector) == a.previous.STANDARD.format(selector=selector)
    ...
    assert a.native.engine.file_digest(Path(a.previous.__file__)) == a.A173_SHA


@pytest.mark.parametrize('core,constraint,selector', [('unknown', 'legacy', 'A'),
    ('legacy', 'unknown', 'B'), ('a171_standard', 'legacy', 'C')])
def test_invalid_system_components_rejected(core, constraint, selector):
    with pytest.raises(ValueError):
        a.system_instruction(core, constraint, selector)


def independent_contrast(records, axis):
    """Enumerate the 16 exact paired differences independently of runtime."""
    ...
    for world, selector, other_level in itertools.product(range(4), ('A', 'B'), ('legacy', 'a171_standard')):
        pair = {row[axis]: row['strict_correct'] for row in records
                if row['world_index'] == world and row['selector'] == selector and row[other] == other_level}
...
def independent_interaction(records):
    """Literal sum of the protocol's eight within-world/selector quartets."""
    values = []
    for world, selector in itertools.product(range(4), ('A', 'B')):
        cells = {(row['instruction_core'], row['closing_constraint']): row['strict_correct']
                 for row in records if row['world_index'] == world and row['selector'] == selector}
...
def test_independent_main_effect_and_secondary_interaction_arithmetic(fixture):
    def assignment(trial):
        core = trial['instruction_core'] == 'a171_standard'
        closing = trial['closing_constraint'] == 'a171_standard'
        return (3 * trial['world_index'] + (trial['selector'] == 'B') + core + 2 * closing) % 7 < 3
    ...
    for axis in AXES:
        ...
        assert contrasts[axis] == {'point': expected, 'lower': expected, 'upper': expected,
                                   'planned_outcomes': 32, 'resolved_outcomes': 32,
                                   'planned_pairs': 16, 'resolved_pairs': 16}
    expected = independent_interaction(records)
    assert a._interaction_bound(records) == {'point': expected, 'lower': expected, 'upper': expected,
        'planned_outcomes': 32, 'resolved_outcomes': 32, 'planned_quartets': 8, 'resolved_quartets': 8}


@pytest.mark.parametrize('axis', AXES)
@pytest.mark.parametrize('direction', [-1, 1])
def test_main_effect_extremes_and_no_spurious_interaction(fixture, axis, direction):
    records = make_records(fixture[0], lambda trial: (trial[axis] == 'a171_standard') == (direction == 1))
    contrasts = a._contrasts(records)
    ...
    assert a._interaction_bound(records)['point'] == 0


@pytest.mark.parametrize('direction', [-1, 1])
def test_secondary_interaction_extrema_are_plus_or_minus_two(fixture, direction):
    records = make_records(fixture[0], lambda trial: (trial['instruction_core'] == trial['closing_constraint']) == (direction == 1))
    interaction = a._interaction_bound(records)
    assert interaction['point'] == interaction['lower'] == interaction['upper'] == 2 * direction
    assert interaction['resolved_quartets'] == 8
    assert all(value['point'] == 0 for value in a._contrasts(records).values())


def test_all_missing_signed_bounds_and_secondary_metadata(fixture):
    records = make_records(fixture[0], lambda trial: None)
    for result in a._contrasts(records).values():
        assert result == {'point': None, 'lower': -1, 'upper': 1, 'planned_outcomes': 32,
                          'resolved_outcomes': 0, 'planned_pairs': 16, 'resolved_pairs': 0}
    assert a._interaction_bound(records) == {'point': None, 'lower': -2, 'upper': 2, 'planned_outcomes': 32,
                          'resolved_outcomes': 0, 'planned_quartets': 8, 'resolved_quartets': 0}
    assert all(row['fence_marker'] is None for row in records)


def test_partial_main_and_interaction_bounds_equal_exhaustive_extremes(fixture):
    def assignment(trial):
        index = trial['sequence_index']
        return None if index in (0, 3) else 'infrastructure' if index == 31 else index % 3 == 0

    records = make_records(fixture[0], assignment)
    unknown_indices = [i for i, row in enumerate(records) if row['strict_correct'] is None]
    assert unknown_indices == [0, 3, 31]
    possible = {key: [] for key in (*AXES, 'interaction')}
    for outcomes in itertools.product((False, True), repeat=3):
        ...
        for axis in AXES:
            possible[axis].append(independent_contrast(resolved, axis))
        possible['interaction'].append(independent_interaction(resolved))
    for axis in AXES:
        actual = a._contrasts(records)[axis]
        assert actual['point'] is None and actual['lower'] == min(possible[axis]) and actual['upper'] == max(possible[axis])
        ...
        assert actual['resolved_outcomes'] == 29 and actual['resolved_pairs'] == 16 - len(unknown_pairs)
    actual = a._interaction_bound(records)
    unknown_quartets = {(records[index]['world_index'], records[index]['selector']) for index in unknown_indices}
    assert actual['point'] is None and actual['lower'] == min(possible['interaction']) and actual['upper'] == max(possible['interaction'])
    assert actual['resolved_outcomes'] == 29 and actual['resolved_quartets'] == 8 - len(unknown_quartets)
    counts = a._counts(records)
    assert counts['categories']['infrastructure'] == 1 and counts['categories']['missing'] == 2
    assert counts['strict_accuracy']['point'] is None and counts['indicators']['fence_marker']['missing'] == 3


@pytest.mark.parametrize('mutation', ['duplicate', 'count', 'integer', 'level', 'quartet'])
def test_invalid_interaction_cohort_rejected(fixture, mutation):
    ...
    if mutation == 'duplicate':
        ...
        records[0]['instruction_core'] = 'unknown'
    else:
        records[0]['world_index'] = 99
    with pytest.raises(ValueError):
        a._interaction_bound(records)


def test_complete_synthetic_schedule_four_groups_and_export_privacy(fixture):
    ...
    assert aggregate['overall']['categories']['correct'] == 32
    assert aggregate['overall']['indicators']['fence_marker'] == {'true': 0, 'false': 32, 'missing': 0}
    for core, closing in itertools.product(*LEVELS):
        cell = aggregate['by_condition'][core][closing]
    ...
    assert aggregate['secondary_contrasts'] == {'strict_accuracy': {'core_by_constraint_interaction': {
        'point': 0, 'lower': 0, 'upper': 0, 'planned_outcomes': 32, 'resolved_outcomes': 32,
        'planned_quartets': 8, 'resolved_quartets': 8}}}
    assert aggregate['claim_boundaries']['interaction_is_secondary'] is True
    assert aggregate['claim_boundaries']['unplanned_interaction_estimands'] is False
    replay = a.export_run(**fixture[1])
    assert replay['aggregate'] == aggregate and len(replay['records']) == 32
...
def test_caps_and_fences_never_gate_or_rescue_accuracy(fixture):
    ...
    def response(trial):
        if trial['instruction_core'] == 'legacy':
            return marker + [ord('x') + 10] * 61
        return encoded_answer(trial, field='selector')

    aggregate, calls = execute(fixture, response=response)
    assert len(calls) == 32 and aggregate['coverage']['completed'] == 32
    assert aggregate['overall']['categories']['cap'] == 16
    assert aggregate['overall']['categories']['selected_label'] == 16
    assert aggregate['overall']['strict_accuracy']['point'] == 0
    assert aggregate['overall']['indicators']['fence_marker'] == {'true': 16, 'false': 16, 'missing': 0}
    assert all(value['point'] == 0 for value in aggregate['contrasts']['strict_accuracy'].values())
    assert aggregate['secondary_contrasts']['strict_accuracy']['core_by_constraint_interaction']['point'] == 0


def test_infrastructure_keeps_fence_unknown_and_all_contrast_points_null(fixture):
    ...
    assert aggregate['secondary_contrasts']['strict_accuracy']['core_by_constraint_interaction']['point'] is None
    assert aggregate['overall']['indicators']['fence_marker'] == {'true': 0, 'false': 31, 'missing': 1}
    assert a.export_run(**fixture[1])['records'][0]['fence_marker'] is None


def test_secondary_interaction_tamper_is_rejected(fixture):
    execute(fixture)
    path = fixture[1]['output_root'] / 'analysis.json'
    value = json.loads(path.read_text())
    value['secondary_contrasts']['strict_accuracy']['core_by_constraint_interaction']['point'] = 1
...
@pytest.mark.parametrize('text,expected', [
    ...
    ('｀｀｀', False), ('～～～', False), ('', False), ('a174_s20', False),
])
def test_only_literal_ascii_fence_flag_is_added_to_pinned_primary_score(fixture, text, expected):
    ...
    original = a.previous.previous.score_tokens(Tokenizer(), tokens, trial, [1, 2])
    score = a.score_tokens(Tokenizer(), tokens, trial, [1, 2])

"""A196 outer qualification: stdlib/fake workers only, no native imports."""
...
SPEC = importlib.util.spec_from_file_location(
    "a196_outer_test", ROOT / "scripts/run_cue_validity_ranking.py"
)
...
def config():
    return {
        "schema_version": "a196-cpu-fp32-config-v1",
...
def frozen_case(tmp_path, monkeypatch):
    ...
    runner = source / "scripts/run_cue_validity_ranking.py"
    runner.parent.mkdir()
    ...
    predecessor_verification = raw_json(
        prepared / "A195-VERIFICATION.json", {"invented_verified": True}
    )
    predecessor_closure = raw_json(prepared / "A195-CLOSED.json", {"invented_closed": True})
    ...
    relative = "scripts/run_cue_validity_ranking.py"
    ...
    monkeypatch.setattr(r, "helper", lambda: Lifecycle())
    frozen = {
        "schema_version": "a196-native-freeze-v1",
...
def test_all_inherited_and_special_inventory_pins_match_actual_source():
    ...
    assert all(
        "answer_path_ranking" not in path and "canonical_answer_ranking" not in path
        for path in r.SOURCE_PATHS
    )


def test_public_runtime_keeps_exact_selected_cpu_policy_and_64_scope():
    value = json.loads((ROOT / "plans/cue_validity_ranking_a196_runtime.public.json").read_bytes())
...
def test_actual_new_core_preparation_metadata_api_without_native_calls(monkeypatch):
    ...
    core = importlib.import_module("lexical_prompt_study.cue_validity_ranking_acquisition")
...
@pytest.mark.parametrize(
    "key,name",
    [
        ("predecessor_verification_sha256", "A195-VERIFICATION.json"),
        ("predecessor_closure_sha256", "A195-CLOSED.json"),
    ],
)
@pytest.mark.parametrize(
...
def test_new_context_endpoints_do_not_change_runtime_or_scoring_accounting():
    # Read prior immutable source as AST only; never import an old study runner.
    import ast

    prior = ast.parse((ROOT / "scripts/run_canonical_answer_ranking.py").read_text())
    current = ast.parse((ROOT / "scripts/run_cue_validity_ranking.py").read_text())
    ...
    assert r.EXECUTION["planned_forwards"] == 64
    assert set(r.SOURCE_PATHS) - set(r.PINS) == {
        "scripts/run_cue_validity_ranking.py",
        "src/lexical_prompt_study/cue_validity_ranking_tasks.py",
        "src/lexical_prompt_study/cue_validity_ranking_acquisition.py",
    }

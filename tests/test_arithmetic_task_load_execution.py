...
SCRIPT = PROJECT / "scripts/run_arithmetic_task_load.py"


def module(path=SCRIPT):
    spec = importlib.util.spec_from_file_location("a187_synthetic_outer", path)
...
def frozen(tmp_path):
    ...
    value = {
        "schema_version": "a187-native-freeze-v1",
    ...
    fresh(path, value)
    return module(source / "scripts/run_arithmetic_task_load.py"), path, value
...
def test_import_is_stdlib_only_and_optimized_run_refuses():
    ...
    assert result.returncode != 0 and b"a187_optimization_forbidden" in result.stderr
...
def fake_worker(tmp_path, monkeypatch):
    ...
    def load(config, engine):
        state["loads"] += 1
        return SimpleNamespace(model=LM(), tokenizer=object(), eos_ids=[9])
    ...
    monkeypatch.setattr(
        m,
        "native_dependencies",
        lambda freeze: (core, engine, SimpleNamespace(load_cpu_reference=load), torch, LM, Decoder),
    )
...
def test_worker_complete_one_load_one_acquisition_replay(fake_worker, monkeypatch):
    ...
    assert terminal["body_entry_counts"] == {"causal_lm_entries": 32, "nested_decoder_entries": 32}
...
def test_active_failed_body_is_retained_and_exception_private(fake_worker):
    ...
    assert failure["body_entry_counts"] == {"causal_lm_entries": 3, "nested_decoder_entries": 3}

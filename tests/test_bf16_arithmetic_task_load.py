"""Invented, model-free A188 constructor, oracle and finite-cohort controls."""
...
from lexical_prompt_study import bf16_arithmetic_task_load as task
        ...
        for item in range(row["item_count"]):
            ...
            for side in (0, 1):
                digest = hashlib.sha256(
                    f"bf16-arithmetic-load-v1|{row['core_index']}|{item}|{side}".encode()
                ).digest()
                n = int(digest.hex(), 16)
...
def test_schedule_independently_hashed_and_typed(rows):
    ...
    ordered = sorted(
        ...
        key=lambda x: (
            hashlib.sha256(
                f"bf16-arithmetic-load-v1|schedule|{x[0]}|{x[1]}|{x[2]}".encode()
            ).hexdigest(),
            *x,
        ),
    )
...
def test_fresh_namespace_and_schema_do_not_relabel_consumed_inputs(rows):
    assert task.NAMESPACE == "bf16-arithmetic-load-v1"
    assert task.SCHEMA == "a188-arithmetic-task-load-v1"
    ...
    for core in range(8):
        ...
        for item in range(8):
            ...
            for side in (0, 1):
                digest = hashlib.sha256(
                    f"arithmetic-load-feasibility-v1|{core}|{item}|{side}".encode()
                ).digest()
    ...
    assert result["schema_version"] == "a188-task-load-analysis-v1"

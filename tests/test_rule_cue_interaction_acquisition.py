"""Invented A197 native-text, arithmetic and durable-receipt controls; no real model."""
...
class CoreTests(unittest.TestCase):
    ...
    def test_fixed_native_geometry_and_shift(self):
        self.assertEqual(len(self.plan["schedule"]), 128)
    ...
    def test_all_four_rule_cue_contexts_share_targets_and_keep_distinct_prompts(self):
        grouped = {}
        for row, obs in zip(self.plan["rows"], self.prepared["observations"], strict=True):
            grouped.setdefault(row["core_index"], {})[(row["rule"], row["condition"])] = (row, obs)
        self.assertEqual(len(grouped), 8)
        for contexts in grouped.values():
            self.assertEqual(
                set(contexts),
                {(r, c) for r in ("copy", "recompute") for c in ("true_cue", "foil_cue")},
            )
            observations = [obs for _, obs in contexts.values()]
            self.assertEqual(len({obs["prompt_utf8"] for obs in observations}), 4)
            self.assertEqual(len({tuple(obs["prompt_token_ids"]) for obs in observations}), 4)
            for candidate in ("arithmetic_truth", "arithmetic_foil"):
                self.assertEqual(
                    len(
                        {
                            tuple(obs["candidates"][candidate]["target_token_ids"])
            ...
            for obs in observations:
                self.assertEqual(
                    set(obs["payload_token_ids"]),
                    {"system", "user", "arithmetic_truth", "arithmetic_foil"},
                )

    def test_old_candidate_key_namespace_is_rejected(self):
        prepared = copy.deepcopy(self.prepared)
        obs = prepared["observations"][0]
        obs["candidates"]["correct"] = obs["candidates"].pop("arithmetic_truth")
        obs["payload_token_ids"]["correct"] = obs["payload_token_ids"].pop("arithmetic_truth")
    ...
    def test_complete_all128_and_model_free_replay(self):
        ...
        self.assertEqual(len(result["context_records"]), 32)
    ...
    def test_repeat_mismatch_retains_all128(self):
        ...
        self.assertEqual(result["summary"]["completed_measurements"], 128)
        self.assertFalse(result["context_records"][0]["numerical_valid"])
        self.assertIsNone(
            result["analysis"]["contrasts"]["margin_nats"]["copy_minus_recompute_cue_effect"][
                "point"
            ]
        )
    ...
    def test_fail_after_three_no_replacement(self):
        ...
        self.assertEqual(value["summary"]["unattempted_measurements"], 124)
...
class ScopeDeltaTests(unittest.TestCase):
    def test_rule_identity_is_hash_bound_in_plan_and_attempts(self):
        plan, prepared, _ = plan_and_prepared()
        self.assertEqual(len(plan["rows"]), 32)
        self.assertEqual(len(plan["schedule"]), 128)
        for slot in plan["schedule"]:
            self.assertIn(slot["rule"], ("copy", "recompute"))
            attempt = core._attempt(plan, prepared, "f" * 64, slot)
            self.assertEqual(attempt["evaluation_sha256"], core.e.object_hash(slot))
        for mutation in ("remove", "replace", "boolean"):
            changed = copy.deepcopy(plan)
            if mutation == "remove":
                changed["schedule"][64].pop("rule")
            else:
                changed["schedule"][64]["rule"] = (
                    False
                    if mutation == "boolean"
                    else "copy"
                    if changed["schedule"][64]["rule"] == "recompute"
                    else "recompute"
                )
            with self.assertRaises(ValueError):
                core.validate_plan(changed)

    def test_prepared_rejects_old_and_off_by_one_context_counts(self):
        plan, prepared, _ = plan_and_prepared()
        for count in (16, 31, 33):
            changed = copy.deepcopy(prepared)
            changed["observations"] = (
                changed["observations"] + [copy.deepcopy(changed["observations"][0])]
            )[:count]
            with self.assertRaises(ValueError):
                core.validate_prepared(plan, changed)

    def test_second_half_call_return_is_required_for_complete_replay(self):
        plan, prepared, binding = plan_and_prepared()
        with tempfile.TemporaryDirectory() as folder:
            ...
            (root / "calls/064/return.json").unlink()
            with self.assertRaises(ValueError):
                core.load_acquisition(root, plan)

    def test_forward_128_ceiling_uses_only_invented_torch_and_python_dispatch(self):
        class Tensor:
            dtype = "long"
        ...
        class Model:
            ...
            def register_forward_pre_hook(self, hook, with_kwargs):
                self._forward_pre_hooks[1] = hook
                self._forward_pre_hooks_with_kwargs[1] = True
                return SimpleNamespace(id=1, remove=lambda: self._forward_pre_hooks.pop(1, None))

            def __call__(self, **kwargs):
                self._forward_pre_hooks[1](self, (), kwargs)
                self.body_calls += 1
                raise RuntimeError("invented post-entry interruption")
        ...
        fake = SimpleNamespace(
            ...
            ones=lambda shape, **kwargs: Tensor([[1] * shape[1]]),
            arange=lambda count, **kwargs: Tensor(list(range(count))),
        ...
        plan, prepared, _ = plan_and_prepared()
        attempt = core._attempt(plan, prepared, "f" * 64, plan["schedule"][127])
        self.assertNotIn("torch", sys.modules)
        for prior in (0, 63, 127, 128):
            with self.subTest(prior=prior), tempfile.TemporaryDirectory() as folder:
                model = Model()
                counts = {"scoring": prior}
                root = Path(folder) / "call"
                with (
                    patch.dict(sys.modules, {"torch": fake}),
                    patch.object(core, "_preflight", lambda *args: None),
                    patch.object(core, "_hook_audit", lambda *args: None),
                ):
                    with self.assertRaisesRegex(
                        ValueError if prior == 128 else RuntimeError,
                        "pre_dispatch_ceiling" if prior == 128 else "post-entry",
                    ):
                        core._forward(model, plan, root, attempt, counts)
                self.assertEqual(model.body_calls, int(prior < 128))
                self.assertEqual(counts["scoring"], prior + int(prior < 128))
                failure = core._read(root / "failure.json")
                self.assertEqual(failure["entries"], int(prior < 128))
                self.assertFalse(failure["usable"])
                self.assertFalse(failure["model_call_returned"])
                self.assertEqual((root / "entry.json").exists(), prior < 128)
                self.assertFalse(model._forward_pre_hooks)
                self.assertFalse(model._forward_pre_hooks_with_kwargs)
        self.assertNotIn("torch", sys.modules)
...
def bad_prepared(change):
    def test(self):
        ...
        if change == "eot_bool":
            ...
            obs["candidates"]["arithmetic_truth"]["target_token_ids"][0] = 20
            obs["candidates"]["arithmetic_truth"]["target_ids_sha256"] = core.e.object_hash(
                obs["candidates"]["arithmetic_truth"]["target_token_ids"]
            )
        elif change == "early_special":
            obs["candidates"]["arithmetic_truth"]["target_token_ids"][0] = 14
            obs["candidates"]["arithmetic_truth"]["target_ids_sha256"] = core.e.object_hash(
                obs["candidates"]["arithmetic_truth"]["target_token_ids"]
            )
        elif change == "last_not_eot":
            obs["candidates"]["arithmetic_truth"]["target_token_ids"][-1] = 12
            ...
            obs["candidates"]["arithmetic_truth"]["target_token_ids"] = [1] * 16 + [13]
            obs["candidates"]["arithmetic_truth"]["target_ids_sha256"] = core.e.object_hash(
                obs["candidates"]["arithmetic_truth"]["target_token_ids"]
            )
        with self.assertRaises(ValueError):
...
class PreparationTests(unittest.TestCase):
    ...
    def test_direct_templates_explicit_false_and_singleton_terminal(self):
        tokenizer = Tokenizer()
        core.prepare_inputs(self.plan, tokenizer)
        self.assertEqual(tokenizer.direct_calls.count(True), 32)
        self.assertEqual(tokenizer.direct_calls.count(False), 64)
    ...
    def test_inventory_mutation_detected_after_all_preparation(self):
        ...
        self.assertEqual(len(tokenizer.direct_calls), 96)

#!/usr/bin/env python3
"""One-shot A187 queue, CPU worker and resource supervisor.
...
if not __debug__:
    raise RuntimeError("a187_optimization_forbidden")
...
SOURCE_PATHS = (
    "scripts/run_arithmetic_task_load.py",
    ...
    "src/lexical_prompt_study/arithmetic_task_load.py",
    "src/lexical_prompt_study/arithmetic_task_load_acquisition.py",
)
PINS = {
    "scripts/run_native_prefix_qualification.py": HELPER_SHA,
    "scripts/run_cpu_reference_audit.py": LOADER_SHA,
...
def require(ok, reason):
    if not ok:
        raise ValueError("a187_outer_" + reason)
...
def helper():
    return _module(
        Path(__file__).with_name("run_native_prefix_qualification.py"), "a187_lifecycle", HELPER_SHA
    )
...
def verify_freeze(path, expected_sha, *, enforce_executable=True):
    ...
    require(frozen["schema_version"] == "a187-native-freeze-v1", "freeze_version")
    ...
    require(
        Path(__file__).absolute() == source / "scripts/run_arithmetic_task_load.py",
...
def expected_runtime_binding(config, config_sha):
    ...
    result["schema_version"] = "a187-runtime-binding-v1"
    ...
    result["note"] = (
        ...
        "A187 uses original CPU FP32 SDPA cache-free greedy generation capped at64."
    )
    return result
...
def native_dependencies(frozen):
    ...
    from lexical_prompt_study import arithmetic_task_load_acquisition as core
    from lexical_prompt_study import instruction_binding_runtime as engine

    imported_sources(frozen)
    loader = _module(source / "scripts/run_cpu_reference_audit.py", "a187_cpu_loader", LOADER_SHA)
    import torch
    from transformers.models.llama.modeling_llama import LlamaForCausalLM, LlamaModel

    return core, engine, loader, torch, LlamaForCausalLM, LlamaModel
    ...
    def __call__(self, frame, event, _arg):
        ...
        if event == "call":
            ...
            publish(
                ...
                {
                    "schema_version": "a187-body-entry-v1",
                    "kind": name,
...
def run_worker(frozen):
    ...
    try:
        ...
        core, engine, loader, torch, lm, decoder = native_dependencies(frozen)
        ...
        with profiler:
            ...
            acquisition_complete(acquired, counts)
            phase = "postflight"
        ...
        if cleanup_errors:
            try:
                ...
                pass
            if original is None:
                raise RuntimeError("a187_worker_cleanup_failed")
...
def validate_worker_terminal(root, expected_freeze_sha):
    ...
    require(
        ...
        == set(
            ...
            "model_free_replay target_released".split()
        ),
...
def supervise(path, expected_sha):
    ...
    try:
        publish(
            root / "claim.json",
            {
                "schema_version": "a187-one-shot-claim-v1",
            ...
            with (root / "worker.log").open("xb") as log:
                process = subprocess.Popen(
                    [
                        NATIVE_PYTHON,
                        "-B",
                        str(Path(frozen["source_root"]) / "scripts/run_arithmetic_task_load.py"),
                        "worker",
                        "--freeze",
                        str(path),
                        "--freeze-sha256",

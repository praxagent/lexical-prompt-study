"""A196 one-shot CPU full-candidate conditional scoring. Import is stdlib-only and inert.
...
if not __debug__:
    raise RuntimeError("a196_optimization_forbidden")

NATIVE_PYTHON = "/data2/PRAX/lexical-prompt-study-data/runtime/a163-local311/bin/python"
SCHEMA = "a196-native-worker-v1"
INSTRUMENT_QUALIFICATION_SHA256 = "228f5fe758022379ac189663153c3063dd5b3238e034b5fd31c8928c20651f47"
PREDECESSOR_VERIFICATION_SHA256 = "e93af368b4b0deb282e3c47b659f50290c62f5d0282625d33675b3d1bbaaf21f"
PREDECESSOR_CLOSURE_SHA256 = "319e9f288e9e7f03f56884779d6e3711789d9085a3c2d0cb1e8accf04c9f71c3"
EXECUTION = {
    "device": "cpu",
    "dtype": "float32",
    "quantization": None,
...
def require(ok, code):
    if not ok:
        raise ValueError("a196_outer_" + code)
...
def helper():
    ...
    return module(Path(__file__).parent / Path(relative).name, "a196_lifecycle", PINS[relative])


def monitor_module():
    relative = "scripts/run_matched_prefix_prediction.py"
    return module(Path(__file__).parent / Path(relative).name, "a196_monitor", PINS[relative])
...
def verify_freeze(path, expected_sha, *, enforce_executable=True):
    ...
    require(
        set(frozen) == FREEZE_KEYS and frozen["schema_version"] == "a196-native-freeze-v1",
        "freeze_schema",
    )
    ...
    require(
        Path(__file__).absolute() == source / "scripts/run_cue_validity_ranking.py",
        "executing_source",
    )
    ...
    for key, name, wanted in (
        (
            ...
            "A195-VERIFICATION.json",
            PREDECESSOR_VERIFICATION_SHA256,
        ),
        ("predecessor_closure_sha256", "A195-CLOSED.json", PREDECESSOR_CLOSURE_SHA256),
    ):
...
def expected_runtime_binding(config, config_sha):
    return {
        "schema_version": "a196-runtime-binding-v1",
...
def validate_config(config):
    ...
    require(
        config["schema_version"] == "a196-cpu-fp32-config-v1"
...
def native_dependencies(frozen):
    ...
    from lexical_prompt_study import cue_validity_ranking_acquisition as core
...
class BodyProfile:
    ...
    def __call__(self, frame, event, arg):
        ...
        publish(
            ...
            {
                "schema_version": "a196-body-entry-v1",
                "kind": name,
        ...
        require(
            self.stage != "nonstudy"
            and self.counts[name] <= 64
            and self.stages[self.stage][name] <= 64,
            "body_ceiling",
        )
...
def complete_counts(acquired, profiler):
    ...
    for kind in ("causal_lm_entries", "nested_decoder_entries"):
        require(
            profiler.counts[kind] == 64
            and profiler.stages["nonstudy"][kind] == 0
            and profiler.stages["science"][kind] == 64,
            "body_dispatch_match",
        )
    ...
    try:
        ...
        if errors:
            try:
                ...
                pass
            if original is None:
                raise RuntimeError("a196_cleanup_failure")
...
def validate_worker_terminal(root, freeze_sha):
    ...
    for kind, n in counts.items():
        require(type(n) is int and n == 64, "body_count")
        require(
...
def supervise(path, expected_sha):
    ...
    try:
        publish(
            root / "claim.json",
            {
                "schema_version": "a196-one-shot-claim-v1",
            ...
            with (root / "worker.log").open("xb") as log:
                process = subprocess.Popen(
                    [
                        NATIVE_PYTHON,
                        "-B",
                        str(Path(frozen["source_root"]) / "scripts/run_cue_validity_ranking.py"),
                        "worker",
                        "--freeze",
                        str(path),
                        "--freeze-sha256",

"""A191 one-shot CPU endpoint acquisition. Import is stdlib-only and inert.
...
PINS = {
    ...
    "src/lexical_prompt_study/matched_prefix_tasks.py": "dd8dc4ddbb5dd0d9a0f3ee61272aae889779218ed7f28dd7eaaeb8110a8e1e94",
    "src/lexical_prompt_study/matched_prefix_acquisition.py": "7676fa455dc4ea1793808912562f16956bec6ec3adb34975cc157b97538ca7de",
}
SOURCE_PATHS = tuple(PINS) + (
    "scripts/run_answer_availability.py",
    "src/lexical_prompt_study/answer_availability_tasks.py",
    "src/lexical_prompt_study/answer_availability_acquisition.py",
)
FREEZE_KEYS = set(
    "schema_version source_root sources target_config_path target_config_sha256 runtime_binding plan native_python limits output_root prepared_root files_sha256 public_commit selection_sha256 qualification_sha256".split()
)


def require(ok, code):
    if not ok:
        raise ValueError("a191_outer_" + code)
...
def helper():
    ...
    return module(Path(__file__).parent / Path(relative).name, "a191_lifecycle", PINS[relative])


def monitor_module():
    relative = "scripts/run_matched_prefix_prediction.py"
    return module(Path(__file__).parent / Path(relative).name, "a191_monitor", PINS[relative])
...
def verify_freeze(path, expected_sha, *, enforce_executable=True):
    ...
    require(
        set(frozen) == FREEZE_KEYS and frozen["schema_version"] == "a191-native-freeze-v1",
        "freeze_schema",
    )
    ...
    require(
        Path(__file__).absolute() == source / "scripts/run_answer_availability.py",
        "executing_source",
    )
...
def expected_runtime_binding(config, config_sha):
    return {
        "schema_version": "a191-runtime-binding-v1",
...
def validate_config(config):
    ...
    require(
        config["schema_version"] == "a191-cpu-fp32-config-v1"
...
def native_dependencies(frozen):
    ...
    from lexical_prompt_study import answer_availability_acquisition as core
...
class BodyProfile:
    def __init__(self, directory, classes):
        ...
        self.stages = {stage: dict(self.counts) for stage in ("nonstudy", "technical", "science")}
        self.stage = "nonstudy"
    ...
    def set_stage(self, stage):
        require(
            (self.stage, stage) in (("nonstudy", "technical"), ("technical", "science")),
            "stage_order",
        )
        if stage == "science":
            require(all(x == 2 for x in self.stages["technical"].values()), "technical_body_count")
        self.stage = stage
    ...
    def __call__(self, frame, event, arg):
        ...
        publish(
            ...
            {
                "schema_version": "a191-body-entry-v1",
                "kind": name,
        ...
        require(
            self.stage != "nonstudy"
            and self.counts[name] <= 1026
            and self.stages[self.stage][name] <= (2 if self.stage == "technical" else 1024),
            "body_ceiling",
        )
...
def complete_counts(acquired, profiler):
    ...
    n = summary["scientific_entries"]
    require(
        type(n) is int
        and 16 <= n <= 1024
        and type(summary["total_entries"]) is int
        and summary["total_entries"] == n + 2,
        "core_entries",
    )
    for kind in ("causal_lm_entries", "nested_decoder_entries"):
        require(
            profiler.counts[kind] == n + 2
            and profiler.stages["nonstudy"][kind] == 0
            and profiler.stages["technical"][kind] == 2
            and profiler.stages["science"][kind] == n,
            "body_dispatch_match",
        )
    ...
    try:
        ...
        if errors:
            try:
                ...
                pass
            if original is None:
                raise RuntimeError("a191_cleanup_failure")
...
def validate_worker_terminal(root, freeze_sha):
    ...
    require(
        ...
        and set(stages) == {"nonstudy", "technical", "science"},
        "body_keys",
    )
    for stage in stages:
    ...
    for kind, n in counts.items():
        require(type(n) is int and 18 <= n <= 1026, "body_count")
        require(
            all(type(stages[s][kind]) is int for s in stages)
            and stages["technical"][kind] == 2
            and stages["nonstudy"][kind] == 0
            and stages["science"][kind] == n - 2,
            "stage_count",
        )
...
def supervise(path, expected_sha):
    ...
    try:
        publish(
            root / "claim.json",
            {
                "schema_version": "a191-one-shot-claim-v1",
            ...
            with (root / "worker.log").open("xb") as log:
                process = subprocess.Popen(
                    [
                        NATIVE_PYTHON,
                        "-B",
                        str(Path(frozen["source_root"]) / "scripts/run_answer_availability.py"),
                        "worker",
                        "--freeze",
                        str(path),
                        "--freeze-sha256",

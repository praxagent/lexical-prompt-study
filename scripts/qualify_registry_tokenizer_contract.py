"""A194 one-shot tokenizer instrument; import is inert and stdlib-only.
...
if not __debug__:
    raise RuntimeError("a194_optimization_forbidden")

SCHEMA = "a194-tokenizer-contract-v1"
...
PINS = {
    ...
    "src/lexical_prompt_study/tokenizer_model_guard.py": "c72ddececed9af8095c7220436e2f345aff314669ad911f07ce54790d61f5cf5",
}
SOURCE_PATHS = tuple(PINS) + ("scripts/qualify_registry_tokenizer_contract.py",)
IMPORT_OPERATIONS = {"dependency_import": 1}
PROHIBITED_KEYS = set(
    "model_construction model_load model_forward gpu_initialization auto_factory_from_config auto_factory_from_pretrained tokenizer_construction tokenizer_load".split()
)
EXPECTED_OPERATIONS = {
    "tokenizer_load": 1,
...
COMMON_FREEZE_KEYS = set(
    "schema_version mode prepared_root source_root sources files_sha256 output_root native_python public_commit selection_sha256 qualification_sha256 protocol_sha256 prior_study_closed prior_closure_sha256 limits expected_operations runtime_versions transformers_root registry_source".split()
)
FREEZE_KEYS = COMMON_FREEZE_KEYS | {"target", "import_qualification_sha256"}
TARGET_KEYS = set(
...
USERS = ("Copy this ordered pair: 23,-4.", "Preserve the order of this pair: 23,-4.")
CANDIDATES = ("23,-4", "24,-4")
DATE = "24 Sep 2026"
EOT = "<|eot_id|>"
...
def helper():
    ...
    return module(Path(__file__).parent / Path(name).name, "a194_lifecycle", PINS[name])


def monitor_module():
    name = "scripts/run_matched_prefix_prediction.py"
    return module(Path(__file__).parent / Path(name).name, "a194_monitor", PINS[name])
...
def validate_import_qualification(value, frozen):
    require(
        type(value) is dict
        and set(value)
        == {
            ...
            "peak_sampled_raw_run_bytes",
            "elapsed_seconds",
            "owned_live_processes_remaining",
            "freeze_sha256",
    ...
    require(
        ...
        and type(value["owned_live_processes_remaining"]) is int
        and value["owned_live_processes_remaining"] == 0
        and is_hash(value["freeze_sha256"])
        and is_hash(value["worker_terminal_sha256"]),
        "artifact_binding",
    )
    require(
        ...
        <= LIMITS["wall_seconds"] + LIMITS["termination_grace_seconds"] + LIMITS["poll_seconds"],
        "artifact_binding",
    )
    for key, bound in (
        ("peak_sampled_owned_rss_bytes", "maximum_owned_rss_bytes"),
        ("peak_sampled_raw_run_bytes", "maximum_raw_run_bytes"),
    ):
        require(type(value[key]) is int and 0 <= value[key] <= LIMITS[bound], "artifact_binding")
    receipt = value.get("import_qualification")
    require(
        ...
        and receipt["schema_version"] == "a194-cold-import-qualification-v1",
        "artifact_binding",
    )
    for key in ("sources", "runtime_versions", "registry_source"):
        require(canonical(receipt[key]) == canonical(frozen[key]), "artifact_binding")
    require(
        canonical(receipt["operations"])
        == canonical(
            {
                "entries": IMPORT_OPERATIONS,
                "returns": IMPORT_OPERATIONS,
    ...
    require(
        set(receipt["prohibited_entries"]) == PROHIBITED_KEYS
        and all(type(v) is int and v == 0 for v in receipt["prohibited_entries"].values()),
        "model_execution",
    )
    validate_guard_summary(receipt["guard"], "import_only")
    require(
        type(receipt["imports"]) is dict
        and set(receipt["imports"])
        == {
            "torch_imported",
            "architecture_implementation_imported",
            "generic_registry_imported",
            "cuda_initialized",
        }
        and type(receipt["imports"]["torch_imported"]) is bool
        and type(receipt["imports"]["generic_registry_imported"]) is bool,
        "artifact_binding",
    )
    require(
        receipt["guard"]["profile_restored"] is True
        and receipt["guard"]["mode"] == "import_only"
        and receipt["imports"]["architecture_implementation_imported"] is False
        and receipt["imports"]["cuda_initialized"] is False,
        "artifact_binding",
    )
...
def verify_freeze(path, expected_sha, *, enforce_executable=True):
    ...
    require(
        type(frozen) is dict and frozen.get("mode") in ("selected", "import_only"), "freeze_schema"
    )
    selected = frozen["mode"] == "selected"
    require(
        set(frozen) == (FREEZE_KEYS if selected else COMMON_FREEZE_KEYS)
        and frozen["schema_version"]
        == ("a194-tokenizer-freeze-v1" if selected else "a194-import-freeze-v1"),
        "freeze_schema",
    )
    ...
    expected_ops = EXPECTED_OPERATIONS if selected else IMPORT_OPERATIONS
    require(
        canonical(frozen["limits"]) == canonical(LIMITS)
        and canonical(frozen["expected_operations"]) == canonical(expected_ops),
        "limits",
    )
    ...
    require(
        Path(frozen["output_root"]) == prepared.parent / ("run-001" if selected else "import-001"),
        "output_path",
    )
    ...
    require(
        Path(__file__).absolute() == source / "scripts/qualify_registry_tokenizer_contract.py",
        "executing_source",
    )
    ...
    require(
        type(frozen["runtime_versions"]) is dict
        and set(frozen["runtime_versions"]) == set(PACKAGES) | {"python"}
        and all(type(x) is str and x for x in frozen["runtime_versions"].values()),
        "runtime",
    )
    require(
        type(frozen["transformers_root"]) is str
        and Path(frozen["transformers_root"]).is_absolute(),
        "source_path",
    )
    guard = guard_module(frozen)
    require(
        canonical(frozen["registry_source"])
        == canonical(
            {"relative_path": guard.REGISTRY_RELATIVE_PATH, "sha256": guard.REGISTRY_SOURCE_SHA256}
        ),
        "source_binding",
    )
    if selected:
        ...
        require(
            ...
            == canonical(frozen["runtime_versions"]),
            "runtime",
        )
        qualification_sha = frozen["import_qualification_sha256"]
        require(
            is_hash(qualification_sha)
            and expected.get("IMPORT-QUALIFICATION.json") == qualification_sha,
            "artifact_binding",
        )
        validate_import_qualification(
            read_json(prepared / "IMPORT-QUALIFICATION.json", qualification_sha), frozen
        )
    return frozen
...
class Ledger:
    ...
    def __init__(self, directory=None, *, expected=None):
        self.directory = Path(directory) if directory is not None else None
        if self.directory is not None:
            self.directory.mkdir(mode=0o700)
        self.expected = dict(expected if expected is not None else EXPECTED_OPERATIONS)
        self.entries = dict.fromkeys(self.expected, 0)
        self.returns = dict.fromkeys(self.expected, 0)
        self.sequence = 0
        self.stage = "startup"
        self.last_operation = None

    def call(self, operation, function, *args, **kwargs):
        require(operation in self.expected, "operation_name")
        require(self.entries[operation] < self.expected[operation], "operation_budget")
        self.sequence += 1
...
def guard_module(frozen):
    name = "src/lexical_prompt_study/tokenizer_model_guard.py"
    return module(Path(frozen["source_root"]) / name, "a194_model_guard", frozen["sources"][name])


def import_report(instrument):
    ...
    return {
        ...
        "architecture_implementation_imported": any(
            n.startswith("transformers.models.")
            and instrument.classify_import(n) == "concrete_architecture"
            for n in sys.modules
        ),
        "generic_registry_imported": instrument.REGISTRY_MODULE in sys.modules,
...
PUBLIC_CLASSIFICATIONS = {
    "architecture_import": "concrete_architecture_blocked",
    "model_construction": "model_execution_blocked",
    "model_load": "model_execution_blocked",
    "model_forward": "model_execution_blocked",
    "auto_factory_from_config": "model_execution_blocked",
    "auto_factory_from_pretrained": "model_execution_blocked",
    "tokenizer_construction": "tokenizer_execution_blocked",
    "tokenizer_load": "tokenizer_execution_blocked",
    "gpu_initialization": "gpu_initialization_blocked",
    "registry_source": "registry_integrity_rejected",
    "registry_search_path": "registry_integrity_rejected",
    "registry_preloaded": "registry_not_cold",
    "invalid_module_name": "invalid_import_name",
    "invalid_mode": "guard_lifecycle_failure",
    "profile_existing": "guard_lifecycle_failure",
    "guard_reuse": "guard_lifecycle_failure",
    "guard_cleanup": "guard_lifecycle_failure",
}


def public_failure_classification(error):
    code = getattr(error, "code", None)
    return (
        PUBLIC_CLASSIFICATIONS.get(code, "operation_failure")
        if type(code) is str
        else "operation_failure"
    )


def failure_code(error, guard=None):
    if type(error) is ContractError:
        return error.code
    if (
        guard is not None and guard.failure_classification is not None
    ) or public_failure_classification(error) != "operation_failure":
        return "dependency_guard_failure"
...
def run_worker(path, expected_sha, *, factory=None):
    ...
    ledger = Ledger(root / "operations", expected=expected_ops)
    guard = None
    evidence = (
        fresh_evidence()
        if selected
    ...
    try:
        ...
        require(canonical(runtime_versions()) == canonical(frozen["runtime_versions"]), "runtime")
        guard_instrument = guard_module(frozen)
        registry = guard_instrument.verify_registry_source(frozen["transformers_root"])
        require(canonical(registry) == canonical(frozen["registry_source"]), "source_binding")
        guard = guard_instrument.NoModelGuard(
            transformers_root=frozen["transformers_root"], mode=frozen["mode"]
        )
        if selected:
            name = "src/lexical_prompt_study/special_token_inventory.py"
            instrument = module(
                Path(frozen["source_root"]) / name,
                "a194_special_token_inventory",
                frozen["sources"][name],
            )
        ...
        with guard:
            require(not import_report(guard_instrument)["cuda_initialized"], "gpu_initialization")
            if selected:
                ledger.stage = "tokenizer_load"
                tokenizer = ledger.call(
                    "tokenizer_load", factory or default_factory, frozen["target"]
                )
                ...
                ledger.stage = "dependency_import"
                imported = ledger.call("dependency_import", factory or default_import)
                require(
                    isinstance(imported, type)
                    and imported.__name__ == "AutoTokenizer"
                    and imported.__module__ == "transformers.models.auto.tokenization_auto",
                    "binding",
                )
                evidence["dependency_returned"] = True
                del imported
            require(
                ledger.entries == expected_ops and ledger.returns == expected_ops,
                "operation_counts",
            )
            require(not any(guard.counts.values()), "model_execution")
            imports = import_report(guard_instrument)
            require(
        ...
        require(
            canonical(guard_instrument.verify_registry_source(frozen["transformers_root"]))
            == canonical(registry),
            "source_changed",
        )
        require(canonical(runtime_versions()) == canonical(frozen["runtime_versions"]), "runtime")
        ...
        summary = {
            ...
            if selected
            else {"planned_imports": 1, "completed_imports": 1, "selected_asset_reads": 0},
            "imports": imports,
            "guard": guard.summary(),
        ...
        try:
            publish(
                root / "worker-failure.json",
                {
                    ...
                    "freeze_sha256": expected_sha,
                    "stage": ledger.stage,
                    "guard_code": failure_code(error, guard),
                    "failure_classification": public_failure_classification(error),
                    "error_type": original_error_type(error),
                    "operations": ledger.snapshot(),
                    "prohibited_entries": dict(guard.counts)
                    if guard
                    else dict.fromkeys(PROHIBITED_KEYS, 0),
                    "checks": completed_checks(evidence)
                    if selected
                    else {
                        "planned_imports": 1,
                        "completed_imports": ledger.returns["dependency_import"],
                        "selected_asset_reads": 0,
                    },
                    "profile_restored": guard.restored if guard else False,
                    "cleanup_error_type": guard.cleanup_error_type if guard else None,
                    "guard": guard.snapshot() if guard else None,
                    "elapsed_seconds": time.monotonic() - started,
                },
            )
        ...
        checks = [
            ...
            (
                "runtime",
                lambda: require(
                    canonical(runtime_versions()) == canonical(frozen["runtime_versions"]),
                    "runtime",
                ),
            ),
        ]
        checks.append(
            (
                ...
                lambda: require(
                    canonical(
                        guard_module(frozen).verify_registry_source(frozen["transformers_root"])
                    )
                    == canonical(frozen["registry_source"]),
                    "source_binding",
                ),
        ...
        if selected:
            checks.append(("assets", lambda: asset_check(frozen["target"])))
...
def validate_guard_summary(value, mode):
    require(
        type(value) is dict
        and set(value)
        == {
            ...
            "mode",
            "prohibited_entries",
            "allowed_registry_requests",
            "failure_classification",
            "profile_restored",
            "cleanup_error_type",
        }
        and value["schema_version"] == "tokenizer-model-guard-v1"
        and value["mode"] == mode
        and type(value["allowed_registry_requests"]) is int
        and value["allowed_registry_requests"] >= 0
        and value["failure_classification"] is None
        and value["profile_restored"] is True
        and value["cleanup_error_type"] is None,
        "terminal_schema",
    )
    require(
        set(value["prohibited_entries"]) == PROHIBITED_KEYS
        and all(type(x) is int and x == 0 for x in value["prohibited_entries"].values()),
        "model_execution",
    )


def validate_worker_terminal(root, freeze_sha, *, mode="selected"):
    root = Path(root)
    ...
    require(
        ...
        and set(summary)
        == set(
            "schema_version mode status freeze_sha256 operations prohibited_entries checks imports guard profile_restored tokenizer_reference_released evidence_sha256 elapsed_seconds".split()
        )
    ...
    require(
        ...
        and type(summary["imports"]["generic_registry_imported"]) is bool
    ...
    require(
        ...
        == canonical(
            {
                "entries": expected_ops,
                "returns": expected_ops,
                "scope": "explicit_caller_operations_not_all_backend_activity",
            }
        ),
        "operation_counts",
    ...
    require(
        set(summary["prohibited_entries"]) == PROHIBITED_KEYS
        and all(type(x) is int and x == 0 for x in summary["prohibited_entries"].values()),
        "model_execution",
    )
    validate_guard_summary(summary["guard"], mode)
    require(
        canonical(summary["guard"]["prohibited_entries"])
        == canonical(summary["prohibited_entries"]),
        "model_execution",
    )
    expected_checks = (
        {"planned_contexts": 2, "completed_contexts": 2, "planned_paths": 4, "completed_paths": 4}
        if selected
        else {"planned_imports": 1, "completed_imports": 1, "selected_asset_reads": 0}
    )
    require(canonical(summary["checks"]) == canonical(expected_checks), "terminal_schema")
    require(
    ...
    require(
        set(header) == {"schema_version", "mode", "freeze_sha256", "identity"}
    ...
    if selected:
        names |= {"assets-before.json", "assets-after.json"}
    require({p.name for p in root.iterdir()} == names, "terminal_inventory")
    if selected:
        require(
        ...
        require(
            canonical(read_json(root / "evidence.json"))
            == canonical(
                {
                    "schema_version": SCHEMA,
                    "mode": "import_only",
                    "dependency": "transformers.AutoTokenizer",
                    "selected_asset_reads": 0,
                    "dependency_returned": True,
                }
            ),
            "terminal_schema",
        )
    counts = dict.fromkeys(expected_ops, 0)
    schedule = operation_schedule() if selected else [("dependency_import", "dependency_import")]
    n = len(schedule)
    require(
    ...
    for i in range(1, n + 1):
        ...
        require(
            ...
            and (entry["operation"], entry["stage"]) == schedule[i - 1],
            "operation_counts",
        )
    require(counts == expected_ops, "operation_counts")
...
def supervise(path, expected_sha, *, mode="selected"):
    frozen = verify_freeze(path, expected_sha)
    require(frozen["mode"] == mode, "freeze_schema")
    lifecycle = helper()
        ...
        with lifecycle.owned_interruptions():
                ...
                process = subprocess.Popen(
                    [
                        ...
                        str(
                            ...
                            / "scripts/qualify_registry_tokenizer_contract.py"
                        ),
                        "worker" if mode == "selected" else "import-worker",
                        "--freeze",
                        str(path),
                        "--freeze-sha256",
                        expected_sha,
            ...
            if result["exit_code"] == 0 and result["stop_reason"] is None:
                ...
                result["worker_terminal_sha256"] = validate_worker_terminal(
                    root / "worker", expected_sha, mode=mode
                )
                result["status"] = "qualified"
                if mode == "import_only":
                    summary = read_json(root / "worker/summary.json")
                    result["import_qualification"] = {
                        "schema_version": "a194-cold-import-qualification-v1",
                        "sources": frozen["sources"],
                        "runtime_versions": frozen["runtime_versions"],
                        "registry_source": frozen["registry_source"],
                        **{
                            key: summary[key]
...
def main():
    ...
    parser.add_argument(
        "mode", choices=("supervise", "worker", "import-supervise", "import-worker")
    )
    ...
    try:
        mode = "import_only" if args.mode.startswith("import-") else "selected"
        require(verify_freeze(args.freeze, args.freeze_sha256)["mode"] == mode, "freeze_schema")
        if args.mode.endswith("worker"):
            ...
            output = supervise(args.freeze, args.freeze_sha256, mode=mode)

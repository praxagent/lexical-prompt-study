"""A195 one-shot CPU full-candidate conditional scoring. Import is stdlib-only and inert.
...
SOURCE_PATHS = tuple(PINS) + (
    "scripts/run_canonical_answer_ranking.py",
    "src/lexical_prompt_study/canonical_answer_ranking_tasks.py",
    "src/lexical_prompt_study/canonical_answer_ranking_acquisition.py",
)
FREEZE_KEYS = set(
    "schema_version source_root sources target_config_path target_config_sha256 runtime_binding plan native_python limits output_root prepared_root files_sha256 public_commit selection_sha256 qualification_sha256 instrument_qualification_sha256".split()
)


def require(ok, code):
    if not ok:
        raise ValueError("a195_outer_" + code)
...
def helper():
    ...
    return module(Path(__file__).parent / Path(relative).name, "a195_lifecycle", PINS[relative])


def monitor_module():
    relative = "scripts/run_matched_prefix_prediction.py"
    return module(Path(__file__).parent / Path(relative).name, "a195_monitor", PINS[relative])
...
def verify_freeze(path, expected_sha, *, enforce_executable=True):
    ...
    require(
        set(frozen) == FREEZE_KEYS and frozen["schema_version"] == "a195-native-freeze-v1",
        "freeze_schema",
    )
    ...
    require(
        Path(__file__).absolute() == source / "scripts/run_canonical_answer_ranking.py",
        "executing_source",
    )
...
def expected_runtime_binding(config, config_sha):
    return {
        "schema_version": "a195-runtime-binding-v1",
...
def validate_config(config):
    ...
    require(
        config["schema_version"] == "a195-cpu-fp32-config-v1"
...
class BodyProfile:
    ...
    def __call__(self, frame, event, arg):
        ...
        publish(
            ...
            {
                "schema_version": "a195-body-entry-v1",
                "kind": name,
    ...
    try:
        ...
        if errors:
            try:
                ...
                pass
            if original is None:
                raise RuntimeError("a195_cleanup_failure")
...
def supervise(path, expected_sha):
    ...
    try:
        publish(
            root / "claim.json",
            {
                "schema_version": "a195-one-shot-claim-v1",
            ...
            with (root / "worker.log").open("xb") as log:
                process = subprocess.Popen(
                    [
                        ...
                        "-B",
                        str(
                            Path(frozen["source_root"]) / "scripts/run_canonical_answer_ranking.py"
                        ),
                        "worker",
                        "--freeze",
                        str(path),

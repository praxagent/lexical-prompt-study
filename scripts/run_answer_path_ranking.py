"""A192 one-shot CPU full-candidate conditional scoring. Import is stdlib-only and inert.
...
SOURCE_PATHS = tuple(PINS) + (
    "scripts/run_answer_path_ranking.py",
    "src/lexical_prompt_study/answer_path_ranking_tasks.py",
    "src/lexical_prompt_study/answer_path_ranking_acquisition.py",
)
FREEZE_KEYS = set(
...
def require(ok, code):
    if not ok:
        raise ValueError("a192_outer_" + code)
...
def helper():
    ...
    return module(Path(__file__).parent / Path(relative).name, "a192_lifecycle", PINS[relative])


def monitor_module():
    relative = "scripts/run_matched_prefix_prediction.py"
    return module(Path(__file__).parent / Path(relative).name, "a192_monitor", PINS[relative])
...
def verify_freeze(path, expected_sha, *, enforce_executable=True):
    ...
    require(
        set(frozen) == FREEZE_KEYS and frozen["schema_version"] == "a192-native-freeze-v1",
        "freeze_schema",
    )
    ...
    require(
        Path(__file__).absolute() == source / "scripts/run_answer_path_ranking.py",
        "executing_source",
    )
...
def expected_runtime_binding(config, config_sha):
    return {
        "schema_version": "a192-runtime-binding-v1",
...
def validate_config(config):
    ...
    require(
        config["schema_version"] == "a192-cpu-fp32-config-v1"
...
class BodyProfile:
    ...
    def __call__(self, frame, event, arg):
        ...
        publish(
            ...
            {
                "schema_version": "a192-body-entry-v1",
                "kind": name,
    ...
    try:
        ...
        if errors:
            try:
                ...
                pass
            if original is None:
                raise RuntimeError("a192_cleanup_failure")
...
def supervise(path, expected_sha):
    ...
    try:
        publish(
            root / "claim.json",
            {
                "schema_version": "a192-one-shot-claim-v1",
            ...
            with (root / "worker.log").open("xb") as log:
                process = subprocess.Popen(
                    [
                        NATIVE_PYTHON,
                        "-B",
                        str(Path(frozen["source_root"]) / "scripts/run_answer_path_ranking.py"),
                        "worker",
                        "--freeze",
                        str(path),
                        "--freeze-sha256",

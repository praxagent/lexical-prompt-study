"""A192 fixed conditional path scoring; injected model/tokenizer, no loader.
...
from . import answer_path_ranking_tasks as tasks

SCHEMA = "a192-answer-path-ranking-acquisition-v1"
MAX_ENTRIES = 64
MAX_PROMPT = 256
MAX_TARGET = 16
...
def _encode(tokenizer, text):
    ...
    require(type(ids) is list, "encoded_ids")
    return ids
...
def prepare_inputs(plan, tokenizer):
    ...
    require(type(eot) is int and eot in plan["eos_token_ids"], "native_eot")
    special = tokenizer.all_special_ids
    e._ids(special, plan["geometry"]["vocab_size"], nonempty=True)
    special = sorted(set(special))
    require(set(plan["eos_token_ids"]) <= set(special), "special_eos")
    observations = []
    for row in plan["rows"]:
        ...
        for ids in payload_ids.values():
            ...
            require(not set(ids) & set(special), "payload_special_token")
        ...
        require(type(prompt) is str and prompt, "rendered_prompt")
        ...
        for key, text in row["candidates"].items():
            ...
            require(completed == prompt + text + EOT, "native_assistant_completion")
            ...
            require(full[: len(ids)] == ids, "joint_prompt_prefix")
            target = full[len(ids) :]
            require(tokenizer.decode(target, **e.DECODER) == text + EOT, "target_roundtrip")
            require(tokenizer.decode(full, **e.DECODER) == completed, "joint_roundtrip")
            candidates[key] = {
                "text": text,
...
def validate_prepared(plan, prepared):
    e._keys(
        prepared,
        "schema_version plan_sha256 eot_token_id special_token_ids observations",
        "prepared_fields",
    )
    require(
    ...
    require(
        type(prepared["observations"]) is list and len(prepared["observations"]) == 16,
        "prepared_rows",
    )
    shared_targets = {}
    for row, obs in zip(plan["rows"], prepared["observations"], strict=True):
        ...
        e._keys(obs["candidates"], "correct first_sum_plus_one", "candidate_keys")
        e._keys(obs["payload_token_ids"], "system user correct first_sum_plus_one", "payload_keys")
        ...
        require(
            ...
            != obs["candidates"]["first_sum_plus_one"]["target_token_ids"],
            "distinct_paths",
        )

"""A177 mapped-value rewrite path; no model calls at import.

All 24 public cells, two primary contrasts and one secondary contrast
...
from . import selection_format_crossover as previous

state, native, tasks, parent = previous.state, previous.native, previous.tasks, previous.parent
object_sha = state.object_sha
A176_SHA = "87bed6b20a0dda0b438c58c411b104c80f9b1efe63f65f9a04711733e9495e5b"
...
PRIVATE_RUNS_ROOT = Path("/data2/PRAX/lexical-prompt-study-data/runs/a177")
MAX_SECONDS, KILL_GRACE_SECONDS = 3600, 60
MAX_NEW_TOKENS = 64
MIN_AVAILABLE_BYTES = 48 * 1024**3
SELECTORS = ("A", "B")
CATEGORIES = previous.CATEGORIES
WORLDS = (("s72", "s79", "AB"), ("s76", "s73", "BA"),
          ("s75", "s78", "BA"), ("s77", "s74", "AB"))
CONDITIONS = ("original_possessive", "of_demonstrative", "of_explicit_label")
BASE_SCHEDULE = tuple(product(CONDITIONS, SELECTORS))
SELECTION_TEXT = "Select the entry labeled {selector} from the user's mapping."
FORMAT_TEXT = {
    "original_possessive": ("Return that entry's mapped value as exactly one JSON object with the single key "
                            '"answer" and a string value.'),
    "of_demonstrative": ('Return the mapped value of that entry as exactly one JSON object with the single key '
                         '"answer" and a string value.'),
    "of_explicit_label": ('Return the mapped value of the entry labeled {selector} as exactly one JSON object '
...
PRIMARY_COMPARISONS = {
    "of_rewrite": ("of_demonstrative", "original_possessive"),
    "explicit_label_rewrite": ("of_explicit_label", "of_demonstrative"),
}
SECONDARY_COMPARISONS = {"endpoint_total": ("of_explicit_label", "original_possessive")}
...
class A177Interrupted(BaseException):
...
class A177Deadline(A177Interrupted):
    """Internal deadline includes preflight and model loading."""


def require(condition, code):
    tasks.require(condition, "a177_" + code)
...
def validate_sources():
    require(native.engine.file_digest(Path(previous.__file__)) == A176_SHA, "a176_helper_source_drift")
...
def system_instruction(condition, selector):
    require(condition in CONDITIONS and selector in SELECTORS, "system_factor_levels")
    return " ".join((SELECTION_TEXT.format(selector=selector),
                     FORMAT_TEXT[condition].format(selector=selector), CLOSING_TEXT))


def _comparison_definition(positive, negative):
    return {"conditions": [positive, negative], "positive": positive, "negative": negative,
...
def compile_plan(config_sha256, protocol_sha256, tests_sha256):
    ...
    for selector in SELECTORS:
        require(system_instruction("original_possessive", selector) == previous.system_instruction(
            "original", "a171_standard", "a171_standard", selector), "exact_original_endpoint_reconstruction")
        require(system_instruction("of_explicit_label", selector) == previous.system_instruction(
            "normalized", "a171_standard", "a171_standard", selector), "exact_explicit_endpoint_reconstruction")
    ...
    for world_index, (value_a, value_b, presentation) in enumerate(WORLDS):
        ...
        for condition, selector in schedule:
            trial = {"world_index": world_index, "world_id": f"w{world_index:02d}", "presentation_order": presentation,
                     ...
                     "messages": [{"role": "system", "content": system_instruction(condition, selector)},
                                  {"role": "user", "content": user}]}
            trial["trial_id"] = object_sha({"schema_version": "a177-cell-identity-v1", "bindings": bindings, "trial": trial})[:24]
            trials.append(trial)
    require(len(trials) == 24 and len({t["trial_id"] for t in trials}) == 24, "fixed_matrix")
    return {"schema_version": "a177-plan-v1", "partition": "fresh_public_synthetic", "bindings": bindings,
            "trials": trials, "world_count": 4, "planned_cells": 24, "maximum_generation_calls": 24,
            ...
            "primary_contrast_definitions": {name: _comparison_definition(*pair) for name, pair in PRIMARY_COMPARISONS.items()},
            "secondary_contrast_definitions": {name: _comparison_definition(*pair) for name, pair in SECONDARY_COMPARISONS.items()},
...
def _inputs(plan_path, plan_sha256, config_path, config_sha256, protocol_path, reference_script, tokenizer_loader):
    ...
    require(len({object_sha(p["prompt_token_ids"]) for p in prepared.values()}) == 24, "unique_native_cells")
...
def prepare_inputs(*, plan_path, plan_sha256, config_path, config_sha256, protocol_path, reference_script,
    ...
    return {"schema_version": "a177-native-freeze-v1", "plan_sha256": plan_sha256, "config_sha256": config_sha256,
            "protocol_sha256": plan["bindings"]["protocol_sha256"], "tests_sha256": plan["bindings"]["tests_sha256"],
            "source_hashes": sources, "reference_script_sha256": REFERENCE_SHA, "planned_cells": 24,
            "maximum_generation_calls": 24, "eos_token_ids": eos,
...
def _private_root(path):
    ...
    require(path.is_relative_to(PRIVATE_RUNS_ROOT) and path != PRIVATE_RUNS_ROOT, "a177_private_run_root")
...
def _attempt(header, trial, prepared):
    return {"schema_version": "a177-attempt-v1", "run_sha256": object_sha(header), "trial_sha256": object_sha(trial),
...
def _startup_attempt(pid, plan_sha256, config_sha256):
    return {"schema_version": "a177-startup-attempt-v1", "pid": pid, "plan_sha256": plan_sha256,
            "config_sha256": config_sha256, "source_sha256": native.engine.file_digest(Path(__file__))}


def _interruption(exc):
    return {"signal_number": exc.signum if isinstance(exc, A177Interrupted) else None,
            "signal_name": exc.signal_name if isinstance(exc, A177Interrupted) else None,
            "pid": exc.pid if isinstance(exc, A177Interrupted) else os.getpid(), "exception_type": type(exc).__name__}
...
def _validate_result(result, attempt, trial, tokenizer, eos):
    require(type(result) is dict and set(result) == {"schema_version", "attempt_sha256", "status", "generated_token_ids",
            "score", "error", "elapsed_seconds"} and result["schema_version"] == "a177-result-v1"
...
def _record(trial, result, attempted):
    ...
    return {"schema_version": "a177-cell-v1", **metadata, **score}
...
def _pair_bound(records, positive, negative):
    """Eight equal world/selector pairs, using only the two required conditions."""
    require(positive in CONDITIONS and negative in CONDITIONS and positive != negative, "comparison_conditions")
    selected = [row for row in records if row["condition"] in (positive, negative)]
    require(len(selected) == 16, "comparison_cohort")
    pairs, terms = {}, []
    for row in selected:
        ...
        require(row["condition"] not in values, "comparison_duplicate_cell")
        values[row["condition"]] = value
        terms.append((value, (1 if row["condition"] == positive else -1) / 8))
    require(len(pairs) == 8 and all(set(values) == {positive, negative} for values in pairs.values()), "comparison_pair_cells")
...
def _contrasts(records):
    return {name: _pair_bound(records, *pair) for name, pair in PRIMARY_COMPARISONS.items()}


def _secondary_contrasts(records):
    return {name: _pair_bound(records, *pair) for name, pair in SECONDARY_COMPARISONS.items()}
...
def _aggregate(plan, records, results, attempted):
    ...
    return {"schema_version": "a177-analysis-v1", "status": "complete" if completed == 24 else "incomplete",
            "planned_worlds": 4, "planned_cells": 24, "maximum_generation_calls": 24,
            "coverage": {"attempted": len(attempted), "completed": completed, "infrastructure_failed": len(results) - completed,
                         "interrupted": len(attempted) - len(results), "unattempted": 24 - len(attempted),
                         "missing": 24 - len(results)},
            ...
            "claim_boundaries": {"fixed_synthetic_worlds_only": True, "confidence_intervals": False,
                ...
                "within_run_adaptation": False, "likelihood_collected": False, "interaction_estimands": False, "endpoint_total_is_secondary": True,
                "cohort_local_missingness": True,
...
def _fixed_header(plan, config, sources, eos, prepared, plan_sha256, config_sha256, reference_script):
    return {"schema_version": "a177-run-v1", "plan_sha256": plan_sha256, "config_sha256": config_sha256,
            ...
            "planned_cells": 24, "maximum_generation_calls": 24, "maximum_seconds": MAX_SECONDS,
...
def _replay(root, plan, config, sources, tokenizer, eos, prepared, plan_sha256, config_sha256, reference_script):
    ...
    require(_same(state._read(root / "one-shot.json"), {"schema_version": "a177-one-shot-v1", "pid": header["pid"],
    ...
    if (root / "execution-finished.json").exists():
        ...
        require(type(finished) is dict and set(finished) == {"schema_version", "status", "elapsed_seconds", "run_sha256", "interruption"}
                and finished["schema_version"] == "a177-execution-finished-v1" and finished["run_sha256"] == object_sha(header)
        ...
        if finished["status"] in ("interrupted", "deadline"):
            require(type(interruption) is dict and set(interruption) == {"signal_number", "signal_name", "pid", "exception_type"}
                    ...
                    ("A177Interrupted", "A177Deadline", "KeyboardInterrupt"), "interruption_receipt")
            allowed = {"A177Interrupted": (signal.SIGTERM, signal.SIGINT, signal.SIGHUP),
                       "A177Deadline": (signal.SIGALRM,), "KeyboardInterrupt": (None,)}
            ...
            require((number is None or type(number) is int) and number in allowed[interruption["exception_type"]] and interruption["signal_name"] ==
                    ...
                    and (finished["status"] == "deadline") == (interruption["exception_type"] == "A177Deadline"), "actual_signal_receipt")
        else:
            require(interruption is None, "unexpected_interruption")
        if finished["status"] == "finished_schedule":
            require(len(results) == 24, "finished_schedule_coverage")
...
def _interrupt(signum, _frame):
    raise A177Interrupted(signum)


def _deadline(signum, _frame):
    raise A177Deadline(signum)
...
def run_plan(*, plan_path, plan_sha256, config_path, config_sha256, protocol_path, reference_script,
    ...
    """One fresh model, all 24 scheduled cells, no outcome gate or retry."""
    ...
    with (root / ".lock").open("a") as lock:
        ...
        with contextlib.ExitStack() as stack:
            try:
                ...
                native.engine.write_private(root / "one-shot.json", {"schema_version": "a177-one-shot-v1", "pid": os.getpid(), "plan_sha256": plan_sha256})
                ...
                for trial in plan["trials"]:
                    ...
                    tick = time.monotonic()
                    result = {"schema_version": "a177-result-v1", "attempt_sha256": object_sha(attempt), "status": "infrastructure_failed",
                    ...
                    try:
                        ...
                        result.update(status="infrastructure_failed", generated_token_ids=None, score=None,
                                      error=native.engine.safe_error(exc, "a177_generation_failed"))
                    result["elapsed_seconds"] = time.monotonic() - tick
                    _validate_result(result, attempt, trial, tokenizer, eos)
                    native.engine.write_private(folder / "result.json", result)
                status = "finished_schedule"
            except (A177Interrupted, KeyboardInterrupt) as exc:
                error = exc
                status = "deadline" if isinstance(exc, A177Deadline) else "interrupted"
                ...
                runtime = None
                if header is None:
                    native.engine.write_private(root / "startup-failure.json", {"schema_version": "a177-startup-failure-v1",
                        ...
                        "error": native.engine.safe_error(error, "a177_startup_failed") if error is not None else None})
                else:
                    if error is not None:
                        native.engine.write_private(root / "error.json", native.engine.safe_error(error, "a177_execution_failed"))
                    native.engine.write_private(root / "execution-finished.json", {"schema_version": "a177-execution-finished-v1", "status": status,
                        "elapsed_seconds": time.monotonic() - started, "run_sha256": object_sha(header), "interruption": interruption})
        if header is None:
            if error is not None:
                raise error
            raise RuntimeError("a177_startup_failed_without_exception")
...
def main(argv=None):
    ...
    except (Exception, A177Interrupted, KeyboardInterrupt) as exc:
        print(json.dumps({"status": "a177_failed", "error_type": type(exc).__name__}))
        return 1

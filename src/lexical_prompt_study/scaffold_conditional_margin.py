"""A180 fixed scaffold-package conditional canonical-path margin study.
...
PRIVATE_RUNS_ROOT = Path("/data2/PRAX/lexical-prompt-study-data/runs/a180")
...
JSON_PREFIX = '{"answer":"'
VALUE_PAIRS = (("s100", "s101"), ("s102", "s103"))
...
PRIMARY_CONTRASTS = {"full_minus_sham_before": ("full_before", "sham_before"),
                     "full_minus_sham_after": ("full_after", "sham_after")}
SECONDARY_CONTRASTS = {"inert_minus_clean_before": ("inert_before", "clean"),
...
class A180Interrupted(BaseException):
...
class A180Deadline(A180Interrupted):
    """Internal deadline includes preflight and model loading."""


def require(condition, code):
    tasks.require(condition, "a180_" + code)
    ...
    for world_index, selectors in enumerate(SELECTOR_ORDERS):
        ...
        for selector_index, selector in enumerate(selectors):
            ...
            for condition in schedule:
                ...
                user = (payload if placement == "none" else materials[kind] + "\n\n" + payload
                        if placement == "before" else payload + "\n\n" + materials[kind])
                context = {"world_index": world_index, "world_id": f"w{world_index:02d}", "pair_index": pair_index,
                    ...
                    "condition": condition, "scaffold_kind": kind, "placement": placement,
                ...
                context["context_id"] = object_sha({"schema_version": "a180-context-identity-v1", "bindings": bindings, "context": context})[:24]
                ...
                for candidate in candidate_order:
                    ...
                    repeats[candidate] += 1
                    evaluation["evaluation_id"] = object_sha({"schema_version": "a180-evaluation-identity-v1", "bindings": bindings,
                                                              "evaluation": evaluation})[:24]
                    evaluations.append(evaluation)
    require(len(contexts) == len({c["context_id"] for c in contexts}) == 56
            and len(evaluations) == len({e["evaluation_id"] for e in evaluations}) == 224, "fixed_matrix")
    return {"schema_version": "a180-plan-v1", "partition": "fresh_mappings_frozen_material_packages", "bindings": bindings,
            ...
            "planned_base_contexts": 8, "planned_contexts": 56, "planned_candidate_paths": 112, "planned_forward_calls": 224,
            ...
            "primary_contrasts": {name: _contrast_definition(*pair) for name, pair in PRIMARY_CONTRASTS.items()},
            ...
            "clean_gate_interpretation_only": True, "scaffold_functional_filter": False, "pooled_primary": False,
...
def _inputs(plan_path, plan_sha256, config_path, config_sha256, protocol_path, reference_script, tokenizer_loader):
    ...
    require(len({object_sha(v["prompt_token_ids"]) for v in prepared.values()}) == 56, "unique_native_contexts")
...
def prepare_inputs(*, plan_path, plan_sha256, config_path, config_sha256, protocol_path, reference_script,
    ...
    return {"schema_version": "a180-native-freeze-v1", "plan_sha256": plan_sha256, "config_sha256": config_sha256,
            "protocol_sha256": plan["bindings"]["protocol_sha256"], "tests_sha256": plan["bindings"]["tests_sha256"],
            "source_hashes": sources, "reference_script_sha256": REFERENCE_SHA, "planned_contexts": 56,
            "planned_forward_calls": 224, "eos_token_ids": eos, "native_prepared_sha256": object_sha(prepared), "native_inputs": prepared}


def _attempt(header, evaluation, prepared):
    value = prepared[evaluation["context_id"]]
    return {"schema_version": "a180-attempt-v1", "run_sha256": object_sha(header), "evaluation": evaluation,
...
def _context_record(context, prepared, evaluations, results):
    ...
    row.update(schema_version="a180-context-margin-v1", **{key: context[key] for key in
               ("base_context_index", "condition", "scaffold_kind", "placement")})
    row["functional_applicable"] = context["condition"] == "clean"
...
def _contrasts(records):
    return {name: _paired_contrast(records, *pair) for name, pair in PRIMARY_CONTRASTS.items()}
...
def _aggregate(plan, records, results, attempted):
    require(len(records) == 56, "fixed_record_count")
    ...
    return {"schema_version": "a180-analysis-v1", "status": "complete" if completed == 224 else "incomplete",
            "planned_value_pairs": 2, "planned_world_assignments": 4, "planned_base_contexts": 8,
            "planned_contexts": 56, "planned_candidate_paths": 112, "planned_forward_calls": 224,
            "coverage": {"attempted": len(attempted), "completed": completed, "infrastructure_failed": len(results) - completed,
                         "interrupted": len(attempted) - len(results), "unattempted": 224 - len(attempted), "missing": 224 - completed},
            ...
            "claim_boundaries": {"a169_reopened": False, "canonical_path_conditional_preference_only": True,
                ...
                "scaffold_functional_filter": False, "pooled_primary": False, "shared_clean_observations": 8,
...
def _fixed_header(plan, config, sources, eos, prepared, plan_sha256, config_sha256, reference_script):
    return {"schema_version": "a180-run-v1", "plan_sha256": plan_sha256, "config_sha256": config_sha256,
            ...
            "planned_contexts": 56, "planned_forward_calls": 224, "maximum_seconds": MAX_SECONDS,
...
def _private_root(path):
    ...
    require(path.is_relative_to(PRIVATE_RUNS_ROOT) and path != PRIVATE_RUNS_ROOT, "a180_private_run_root")
...
def _startup_attempt(pid, plan_sha256, config_sha256):
    return {"schema_version": "a180-startup-attempt-v1", "pid": pid, "plan_sha256": plan_sha256,
            "config_sha256": config_sha256, "source_sha256": native.engine.file_digest(Path(__file__))}


def _interruption(exc):
    return {"signal_number": exc.signum if isinstance(exc, A180Interrupted) else None,
            "signal_name": exc.signal_name if isinstance(exc, A180Interrupted) else None,
            "pid": exc.pid if isinstance(exc, A180Interrupted) else os.getpid(), "exception_type": type(exc).__name__}
...
def _validate_result(result, attempt):
    require(type(result) is dict and set(result) == {"schema_version", "attempt_sha256", "status", "measurement", "error", "elapsed_seconds"}
            and result["schema_version"] == "a180-result-v1" and result["attempt_sha256"] == object_sha(attempt)
...
def _replay(root, plan, config, sources, tokenizer, eos, prepared, plan_sha256, config_sha256, reference_script):
    ...
    require(_same(state._read(root / "one-shot.json"), {"schema_version": "a180-one-shot-v1", "pid": header["pid"],
    ...
    if (root / "execution-finished.json").exists():
        ...
        require(type(finished) is dict and set(finished) == {"schema_version", "status", "elapsed_seconds", "run_sha256", "interruption"}
                and finished["schema_version"] == "a180-execution-finished-v1" and finished["run_sha256"] == object_sha(header)
        ...
        if finished["status"] in ("interrupted", "deadline"):
            require(type(interruption) is dict and set(interruption) == {"signal_number", "signal_name", "pid", "exception_type"}
                    ...
                    ("A180Interrupted", "A180Deadline", "KeyboardInterrupt"), "interruption_receipt")
            allowed = {"A180Interrupted": (signal.SIGTERM, signal.SIGINT, signal.SIGHUP),
                       "A180Deadline": (signal.SIGALRM,), "KeyboardInterrupt": (None,)}
            ...
            require((number is None or type(number) is int) and number in allowed[interruption["exception_type"]] and interruption["signal_name"] ==
                    ...
                    and (finished["status"] == "deadline") == (interruption["exception_type"] == "A180Deadline"), "actual_signal_receipt")
        else:
            require(interruption is None, "unexpected_interruption")
        if finished["status"] == "finished_schedule":
            require(len(results) == 224, "finished_schedule_coverage")
...
def _interrupt(signum, _frame):
    raise A180Interrupted(signum)


def _deadline(signum, _frame):
    raise A180Deadline(signum)
...
def run_plan(*, plan_path, plan_sha256, config_path, config_sha256, protocol_path, reference_script,
    ...
    """One fresh model, all 224 scheduled forwards, no outcome gate or retry."""
    ...
    with (root / ".lock").open("a") as lock:
        ...
        with contextlib.ExitStack() as stack:
            try:
                ...
                native.engine.write_private(root / "one-shot.json", {"schema_version": "a180-one-shot-v1", "pid": os.getpid(), "plan_sha256": plan_sha256})
                ...
                for evaluation in plan["evaluations"]:
                    ...
                    tick = time.monotonic()
                    result = {"schema_version": "a180-result-v1", "attempt_sha256": object_sha(attempt), "status": "infrastructure_failed",
                    ...
                    try:
                        ...
                        result.update(status="infrastructure_failed", measurement=None,
                                      error=native.engine.safe_error(exc, "a180_measurement_failed"))
                    result["elapsed_seconds"] = time.monotonic() - tick
                    _validate_result(result, attempt)
                    native.engine.write_private(folder / "result.json", result)
                status = "finished_schedule"
            except (A180Interrupted, KeyboardInterrupt) as exc:
                error = exc
                status = "deadline" if isinstance(exc, A180Deadline) else "interrupted"
                ...
                runtime = None
                if header is None:
                    native.engine.write_private(root / "startup-failure.json", {"schema_version": "a180-startup-failure-v1",
                        ...
                        "error": native.engine.safe_error(error, "a180_startup_failed") if error is not None else None})
                else:
                    if error is not None:
                        native.engine.write_private(root / "error.json", native.engine.safe_error(error, "a180_execution_failed"))
                    native.engine.write_private(root / "execution-finished.json", {"schema_version": "a180-execution-finished-v1", "status": status,
                        "elapsed_seconds": time.monotonic() - started, "run_sha256": object_sha(header), "interruption": interruption})
        if header is None:
            if error is not None:
                raise error
            raise RuntimeError("a180_startup_failed_without_exception")
...
def main(argv=None):
    ...
    except (Exception, A180Interrupted, KeyboardInterrupt) as exc:
        print(json.dumps({"status": "a180_failed", "error_type": type(exc).__name__}))
        return 1

"""A187 fixed generation-only arithmetic task-load acquisition.

Inject a qualified CPU FP32 model/tokenizer; no asset loading or work on import.
...
is performed. The pinned A186 forward subreceipt schema is explicitly retained.
"""
...
from . import arithmetic_task_load as tasks
...
SCHEMA = "a187-arithmetic-acquisition-v1"
CAP, MAX_PROMPT, MAX_ENTRIES, PREFIX_TOKENS, ROWS = 64, 256, 2048, 8, 32
DATE = "26 Jul 2024"
...
A186_SHA = "7676fa455dc4ea1793808912562f16956bec6ec3adb34975cc157b97538ca7de"
_write, _read = previous._write, previous._read


def require(ok, reason):
    if not ok:
        raise ValueError("a187_acquisition_" + reason)
...
def source_bindings():
    pins = ((e, A183_SHA), (controls, A184_SHA), (previous, A185_SHA), (primitive, A186_SHA))
    ...
    return {
        ...
        "a185": A185_SHA,
        "a186": A186_SHA,
        "a186_tasks": e.sha256(Path(primitive.tasks.__file__).read_bytes()),
...
def compile_plan(
    *, provenance, geometry, eos_token_ids, runtime_binding_sha256, protocol_sha256, tests_sha256
    ...
    return {
        ...
        "study_id": "a187_arithmetic_task_load",
        "rows": rows,
...
def _scan_steps(plan, row, observation, root, run_sha, *, complete):
    ...
    for index, name in enumerate(names):
        ...
        if (step / "failure.json").exists():
            ...
            allowed = {"attempt.json", "entry.json", "result.json", "logits.fp32", "failure.json"}
            require(
            ...
            allowed = {"attempt.json", "entry.json", "logits.fp32"}
            require(
                files
                <= allowed
...
def run_acquisition(model, tokenizer, *, plan, prepared, runtime_binding, directory):
    ...
    previous._clean_model(model, _primitive_plan(plan))
    import torch

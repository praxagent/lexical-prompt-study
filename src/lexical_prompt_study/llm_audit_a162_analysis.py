"""Prospective A162 OpenAI receipt validation and numeric model-judge concordance.
...
A162 freezes and rubric-bearing configs bind the prospective instrument. The
provider engine's unchanged low-level receipt wire format is an explicit reuse.
"""
...
def validate_freeze(freeze: dict, expected_sha256: str) -> None:
    ...
    require(freeze["schema_version"] == "a162-freeze-v1", "openai_freeze_version")
    require(freeze["instrument_id"] == "a162", "a162_instrument")
    choice(freeze["stage"], ("development", "holdout", "actual"), "a162_stage")
    ...
    for field in ("protocol_sha256", "source_sha256", "legacy_source_sha256", "packet_sha256", "fixture_sha256"):
        digest(freeze[field])
    for field in ("engine_source_sha256", "rubric_sha256", "development_fixture_sha256",
                  "holdout_fixture_sha256", "budget_protocol_sha256"):
        digest(freeze[field])
    require(freeze["legacy_source_sha256"] == LEGACY_SHA256
            and freeze["engine_source_sha256"] == ENGINE_SHA256
    ...
    for field in ("rubric_path", "holdout_fixture_path", "development_fixture_path"):
        require(type(freeze[field]) is str and str(_safe_path(Path(freeze[field]))) == freeze[field],
                "a162_dependency_path")
    active_fixture = (freeze["development_fixture_sha256"] if freeze["stage"] == "development"
                      else freeze["holdout_fixture_sha256"])
    require(freeze["fixture_sha256"] == active_fixture, "a162_stage_fixture")
    require((freeze["stage"] == "actual") == (freeze["scope"] == "verified_a156"), "a162_stage_scope")
    ...
    for field in ("output_root", "budget_root"):
        require(type(freeze[field]) is str and str(_safe_path(Path(freeze[field]))) == freeze[field],
                "openai_absolute_root")
    require(type(freeze["run_id"]) is str
            and re.fullmatch(r"a162-[a-z0-9][a-z0-9_-]{0,57}", freeze["run_id"]),
            "openai_run_id")
    ...
    require(freeze["budget_root"] == BUDGET_ROOT and freeze["budget_protocol_sha256"] == BUDGET_PROTOCOL_SHA256,
            "a162_existing_budget")
    integer(freeze["budget_micro_usd"], 10_000_000, 10_000_000, "openai_budget_cap")
    integer(freeze["input_token_cap"], 5_000_000, 5_000_000, "openai_input_cap")
    ...
    require(type(selection) is list and len(selection) == expected_count, "openai_selection")
    ...
    if freeze["stage"] == "development":
        ...
        require(freeze["qualification_sha256"] != "0" * 64
                ...
                and str(_safe_path(Path(freeze["qualification_path"]))) == freeze["qualification_path"],
                "openai_qualification_binding")
    if freeze["stage"] == "actual":
        require(freeze["packet_sha256"] == A156_PACKET_SHA256 and len(selection) == 287,
                "openai_actual_selection")
    else:
        require(freeze["packet_sha256"] != A156_PACKET_SHA256, "a162_synthetic_packet")
    config = freeze["config"]
    keys(config, CONFIG_FIELDS, "openai_config_fields")
    require(config["instrument_id"] == "a162" and config["rubric_sha256"] == freeze["rubric_sha256"],
            "a162_config_rubric")
...
def summarize_runs(runs: list[dict], numeric_mapping: dict | None = None,
    ...
    for run in runs:
        ...
        if baseline is None:
            ...
            require(all(freeze[field] == baseline[field] for field in (
                ...
                "instrument_id", "stage", "engine_source_sha256", "rubric_sha256",
                "development_fixture_sha256", "holdout_fixture_sha256", "budget_protocol_sha256",
                "budget_root", "budget_micro_usd", "input_token_cap",
        ...
        judges.append({
            ...
            "instrument_id": "a162", "stage": freeze["stage"], "rubric_sha256": freeze["rubric_sha256"],
            ...
            "engine_source_sha256": freeze["engine_source_sha256"],
    ...
    result = {
        "schema_version": "a162-audit-summary-v1", "provider": "openai", "judge_kind": "llm",
        "instrument_id": "a162", "stage": baseline["stage"], "rubric_sha256": baseline["rubric_sha256"],
        "development_fixture_sha256": baseline["development_fixture_sha256"],
        "holdout_fixture_sha256": baseline["holdout_fixture_sha256"],
        "budget_protocol_sha256": baseline["budget_protocol_sha256"],
        ...
        "claim_boundaries": {
            ...
            "a162_instrument_with_unchanged_provider_engine_wire_format": True,
            "development_holdout_and_actual_stages_not_pooled": True,
...
def load_run(freeze_path: Path, expected_freeze_sha256: str) -> dict:
    ...
    from . import llm_audit_a162 as instrument_runner
    require(hashlib.sha256(Path(instrument_runner.__file__).read_bytes()).hexdigest()
            == freeze["source_sha256"], "a162_replay_instrument_source")
    require(hashlib.sha256(Path(provider_runner.__file__).read_bytes()).hexdigest()
            == freeze["engine_source_sha256"], "a162_replay_engine_source")
    ...
    try:
        ...
        require(read_json(root / "manifest.json") == {
            "schema_version": "a162-store-v1", "instrument_id": "a162", "freeze_sha256": expected_freeze_sha256,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        require(spec["schema_version"] == "a162-analysis-inputs-v1", "openai_analysis_inputs_version")
        ...
        print('{"status":"a162_analysis_rejected"}')
        return 1

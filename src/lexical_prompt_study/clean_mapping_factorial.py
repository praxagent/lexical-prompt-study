"""A173 clean-only mapping factorial; no model calls at import.

All 64 public synthetic cells and three accuracy contrasts are fixed before
outcomes. Raw generation evidence remains private; stdout is coverage only.
"""
...
from . import mapped_value_clarification as previous

state, native, tasks, parent = previous.state, previous.native, previous.tasks, previous.parent
object_sha = state.object_sha
A171_SHA = "389bdbd9228d10c83c4da0ff90ace6592cb31b9abc25d3aee4bb1956483fc223"
...
PRIVATE_RUNS_ROOT = Path("/data2/PRAX/lexical-prompt-study-data/runs/a173")
MAX_SECONDS, KILL_GRACE_SECONDS = 7200, 60
MAX_NEW_TOKENS = 64
MIN_AVAILABLE_BYTES = 48 * 1024**3
SYSTEM_WORDINGS, MAPPING_HEADINGS = ("legacy", "a171_standard"), ("off", "on")
VALUE_FORMS, SELECTORS = ("short_symbol", "study_prefixed"), ("A", "B")
AXES = ("system_wording", "mapping_heading", "value_form")
LEVELS = (SYSTEM_WORDINGS, MAPPING_HEADINGS, VALUE_FORMS)
CATEGORIES = previous.CATEGORIES
WORLDS = (("s20", "s27", "AB"), ("s24", "s21", "BA"),
          ("s23", "s26", "BA"), ("s25", "s22", "AB"))
BASE_SCHEDULE = tuple(product(SYSTEM_WORDINGS, MAPPING_HEADINGS, VALUE_FORMS, SELECTORS))
...
class A173Interrupted(BaseException):
...
class A173Deadline(A173Interrupted):
    """Internal deadline includes preflight and model loading."""


def require(condition, code):
    tasks.require(condition, "a173_" + code)
...
def validate_sources():
    require(native.engine.file_digest(Path(previous.__file__)) == A171_SHA, "a171_helper_source_drift")
...
def compile_plan(config_sha256, protocol_sha256, tests_sha256):
    """Compile only the fixed public 4-world factorial; no parent evidence reads."""
    ...
    bindings = {"config_sha256": config_sha256, "protocol_sha256": protocol_sha256, "tests_sha256": tests_sha256,
                ...
                "a171_helper_sha256": A171_SHA}
    trials = []
    for world_index, (value_a, value_b, presentation) in enumerate(WORLDS):
        rotation = 8 * (world_index // 2)
        schedule = BASE_SCHEDULE[rotation:] + BASE_SCHEDULE[:rotation]
        if world_index % 2:
            schedule = tuple(reversed(schedule))
        for system_wording, mapping_heading, value_form, selector in schedule:
            prefix = "a173_" if value_form == "study_prefixed" else ""
            mapping = {"A": prefix + value_a, "B": prefix + value_b}
            user = json.dumps({key: mapping[key] for key in presentation}, separators=(",", ":"))
            if mapping_heading == "on":
                user = "Mapping:\n" + user
            system = tasks.system_instruction(selector) if system_wording == "legacy" else STANDARD.format(selector=selector)
            trial = {"world_index": world_index, "world_id": f"w{world_index:02d}", "presentation_order": presentation,
                     "system_wording": system_wording, "mapping_heading": mapping_heading, "value_form": value_form,
                     "selector": selector, "selected_answer": mapping[selector],
                     "unselected_answer": mapping["B" if selector == "A" else "A"], "sequence_index": len(trials),
                     "messages": [{"role": "system", "content": system}, {"role": "user", "content": user}]}
            trial["trial_id"] = object_sha({"schema_version": "a173-cell-identity-v1", "bindings": bindings, "trial": trial})[:24]
            trials.append(trial)
    require(len(trials) == 64 and len({t["trial_id"] for t in trials}) == 64, "fixed_matrix")
    return {"schema_version": "a173-plan-v1", "partition": "fresh_public_synthetic", "bindings": bindings,
            "trials": trials, "world_count": 4, "planned_cells": 64, "maximum_generation_calls": 64,
            ...
            "prose_allowed": False, "clarification_reminder_allowed": False,
            "factor_levels": {axis: list(levels) for axis, levels in zip(AXES, LEVELS)},
...
def _inputs(plan_path, plan_sha256, config_path, config_sha256, protocol_path, reference_script, tokenizer_loader):
    ...
    require(len({object_sha(p["prompt_token_ids"]) for p in prepared.values()}) == 64, "unique_native_cells")
...
def prepare_inputs(*, plan_path, plan_sha256, config_path, config_sha256, protocol_path, reference_script,
    ...
    return {"schema_version": "a173-native-freeze-v1", "plan_sha256": plan_sha256, "config_sha256": config_sha256,
            "protocol_sha256": plan["bindings"]["protocol_sha256"], "tests_sha256": plan["bindings"]["tests_sha256"],
            "source_hashes": sources, "reference_script_sha256": REFERENCE_SHA, "planned_cells": 64,
            "maximum_generation_calls": 64, "eos_token_ids": eos,
            "native_prepared_sha256": object_sha(prepared), "native_inputs": prepared}


def score_tokens(tokenizer, tokens, trial, eos_ids):
    """Preserve the pinned primary score and add only the fixed fence marker."""
    score = previous.score_tokens(tokenizer, tokens, trial, eos_ids)
    text = score["response_text"]
    return {**score, "fence_marker": "```" in text or "~~~" in text}
...
def _private_root(path):
    ...
    require(path.is_relative_to(PRIVATE_RUNS_ROOT) and path != PRIVATE_RUNS_ROOT, "a173_private_run_root")
...
def _attempt(header, trial, prepared):
    return {"schema_version": "a173-attempt-v1", "run_sha256": object_sha(header), "trial_sha256": object_sha(trial),
...
def _startup_attempt(pid, plan_sha256, config_sha256):
    return {"schema_version": "a173-startup-attempt-v1", "pid": pid, "plan_sha256": plan_sha256,
            "config_sha256": config_sha256, "source_sha256": native.engine.file_digest(Path(__file__))}


def _interruption(exc):
    return {"signal_number": exc.signum if isinstance(exc, A173Interrupted) else None,
            "signal_name": exc.signal_name if isinstance(exc, A173Interrupted) else None,
            "pid": exc.pid if isinstance(exc, A173Interrupted) else os.getpid(), "exception_type": type(exc).__name__}
...
def _validate_result(result, attempt, trial, tokenizer, eos):
    require(type(result) is dict and set(result) == {"schema_version", "attempt_sha256", "status", "generated_token_ids",
            "score", "error", "elapsed_seconds"} and result["schema_version"] == "a173-result-v1"
...
def _record(trial, result, attempted):
    metadata = {key: trial[key] for key in ("trial_id", "sequence_index", "world_index", "world_id", "presentation_order",
                                           "system_wording", "mapping_heading", "value_form", "selector")}
    ...
    return {"schema_version": "a173-cell-v1", **metadata, **score}


def _binary_bound(records, field, axis):
    """Equal-weight 32-pair main effect, with sharp signed missingness bounds."""
    require(axis in AXES and len(records) == 64, "contrast_axis_and_cohort")
    ...
    for row in records:
        ...
        terms.append((row[field], (1 if row[axis] == levels[1] else -1) / 32))
    require(len(pairs) == 32 and all(set(values) == set(levels) for values in pairs.values()), "paired_contrast_cells")
    ...
    return {"point": fixed if not unresolved else None,
            ...
            "planned_outcomes": 64, "resolved_outcomes": len(known), "planned_pairs": 32,
...
def _aggregate(plan, records, results, attempted):
    ...
    return {"schema_version": "a173-analysis-v1", "status": "complete" if completed == 64 else "incomplete",
            "planned_worlds": 4, "planned_cells": 64, "maximum_generation_calls": 64,
            "coverage": {"attempted": len(attempted), "completed": completed, "infrastructure_failed": len(results) - completed,
                         "interrupted": len(attempted) - len(results), "unattempted": 64 - len(attempted),
                         "missing": 64 - len(results)},
            "overall": _counts(records),
            "by_condition": {system: {heading: {form: _counts([r for r in records if r["system_wording"] == system
                and r["mapping_heading"] == heading and r["value_form"] == form]) for form in VALUE_FORMS}
                for heading in MAPPING_HEADINGS} for system in SYSTEM_WORDINGS},
            ...
            "claim_boundaries": {"fixed_synthetic_worlds_only": True, "confidence_intervals": False,
                ...
                "within_run_adaptation": False, "likelihood_collected": False, "interaction_estimands": False,
                "fence_is_accuracy_rescue": False, "isolated_prefix_or_wording_mechanism": False}}


def _fixed_header(plan, config, sources, eos, prepared, plan_sha256, config_sha256, reference_script):
    return {"schema_version": "a173-run-v1", "plan_sha256": plan_sha256, "config_sha256": config_sha256,
            ...
            "planned_cells": 64, "maximum_generation_calls": 64, "maximum_seconds": MAX_SECONDS,
...
def _replay(root, plan, config, sources, tokenizer, eos, prepared, plan_sha256, config_sha256, reference_script):
    ...
    require(_same(state._read(root / "one-shot.json"), {"schema_version": "a173-one-shot-v1", "pid": header["pid"],
    ...
    if (root / "execution-finished.json").exists():
        ...
        require(type(finished) is dict and set(finished) == {"schema_version", "status", "elapsed_seconds", "run_sha256", "interruption"}
                and finished["schema_version"] == "a173-execution-finished-v1" and finished["run_sha256"] == object_sha(header)
        ...
        if finished["status"] in ("interrupted", "deadline"):
            require(type(interruption) is dict and set(interruption) == {"signal_number", "signal_name", "pid", "exception_type"}
                    ...
                    ("A173Interrupted", "A173Deadline", "KeyboardInterrupt"), "interruption_receipt")
            allowed = {"A173Interrupted": (signal.SIGTERM, signal.SIGINT, signal.SIGHUP),
                       "A173Deadline": (signal.SIGALRM,), "KeyboardInterrupt": (None,)}
            ...
            require(number in allowed[interruption["exception_type"]] and interruption["signal_name"] ==
                    ...
                    and (finished["status"] == "deadline") == (interruption["exception_type"] == "A173Deadline"), "actual_signal_receipt")
        else:
            require(interruption is None, "unexpected_interruption")
        if finished["status"] == "finished_schedule":
            require(len(results) == 64, "finished_schedule_coverage")
...
def _interrupt(signum, _frame):
    raise A173Interrupted(signum)


def _deadline(signum, _frame):
    raise A173Deadline(signum)
...
def run_plan(*, plan_path, plan_sha256, config_path, config_sha256, protocol_path, reference_script,
    ...
    """One fresh model, all 64 scheduled cells, no outcome gate or retry."""
    ...
    with (root / ".lock").open("a") as lock:
        ...
        with contextlib.ExitStack() as stack:
            try:
                ...
                native.engine.write_private(root / "one-shot.json", {"schema_version": "a173-one-shot-v1", "pid": os.getpid(), "plan_sha256": plan_sha256})
                ...
                for trial in plan["trials"]:
                    ...
                    tick = time.monotonic()
                    result = {"schema_version": "a173-result-v1", "attempt_sha256": object_sha(attempt), "status": "infrastructure_failed",
                    ...
                    try:
                        ...
                        result.update(status="infrastructure_failed", generated_token_ids=None, score=None,
                                      error=native.engine.safe_error(exc, "a173_generation_failed"))
                    result["elapsed_seconds"] = time.monotonic() - tick
                    _validate_result(result, attempt, trial, tokenizer, eos)
                    native.engine.write_private(folder / "result.json", result)
                status = "finished_schedule"
            except (A173Interrupted, KeyboardInterrupt) as exc:
                error = exc
                status = "deadline" if isinstance(exc, A173Deadline) else "interrupted"
                ...
                runtime = None
                if header is None:
                    native.engine.write_private(root / "startup-failure.json", {"schema_version": "a173-startup-failure-v1",
                        ...
                        "error": native.engine.safe_error(error, "a173_startup_failed") if error is not None else None})
                else:
                    if error is not None:
                        native.engine.write_private(root / "error.json", native.engine.safe_error(error, "a173_execution_failed"))
                    native.engine.write_private(root / "execution-finished.json", {"schema_version": "a173-execution-finished-v1", "status": status,
                        "elapsed_seconds": time.monotonic() - started, "run_sha256": object_sha(header), "interruption": interruption})
        if header is None:
            if error is not None:
                raise error
            raise RuntimeError("a173_startup_failed_without_exception")
...
def main(argv=None):
    ...
    except (Exception, A173Interrupted, KeyboardInterrupt) as exc:
        print(json.dumps({"status": "a173_failed", "error_type": type(exc).__name__}))
        return 1

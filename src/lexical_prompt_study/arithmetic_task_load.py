"""A187 fixed task-load construction, finite oracle and aggregate-only analysis.
...
NAMESPACE = "arithmetic-load-feasibility-v1"
SCHEMA = "a187-arithmetic-task-load-v1"
CORE_COUNT = 8
ITEM_COUNTS = (4, 8)
...
SYSTEMS = {
    ...
    for count, word in ((4, "four"), (8, "eight"))
}
...
def operand(core_index, item_index, side, magnitude):
    _core(core_index)
    _require(type(item_index) is int and 0 <= item_index < 8, "item_index")
    _require(type(side) is int and side in (0, 1), "side")
    _condition(4, magnitude)
...
def aggregate_records(rows, records):
    ...
    return {
        "schema_version": "a187-task-load-analysis-v1",
        ...
        "contrasts": {
            "success_rate": {"shorter_minus_longer": _contrast(records, "item_count", 4, 8)}
        },
        "secondary_contrasts": {
            "success_rate": {

"""A174 closing-constraint crossover; no model calls at import.

All 32 public synthetic cells, two primary contrasts and one secondary
interaction are fixed before outcomes. Raw generation evidence remains private; stdout is coverage only.
"""
...
from . import clean_mapping_factorial as previous

state, native, tasks, parent = previous.state, previous.native, previous.tasks, previous.parent
object_sha = state.object_sha
A173_SHA = "9d197e0c91e63bfcc51b70016a19e1c7ac477ebecfc3e12405f594202413e075"
...
PRIVATE_RUNS_ROOT = Path("/data2/PRAX/lexical-prompt-study-data/runs/a174")
...
LEVELS = (INSTRUCTION_CORES, CLOSING_CONSTRAINTS)
CATEGORIES = previous.CATEGORIES
WORLDS = (("s40", "s47", "AB"), ("s44", "s41", "BA"),
          ("s43", "s46", "BA"), ("s45", "s42", "AB"))
BASE_SCHEDULE = tuple(product(INSTRUCTION_CORES, CLOSING_CONSTRAINTS, SELECTORS))
CORE_TEXT = {
    ...
    "a171_standard": ("Select the entry labeled {selector} from the user's mapping. Return that entry's mapped value as "
                      'exactly one JSON object with the single key "answer" and a string value.'),
}
CLOSING_TEXT = {
    "legacy": "Do not transform the value or include other fields, explanations, or Markdown fences.",
    "a171_standard": "Do not return any other text.",
}
...
class A174Interrupted(BaseException):
...
class A174Deadline(A174Interrupted):
    """Internal deadline includes preflight and model loading."""


def require(condition, code):
    tasks.require(condition, "a174_" + code)
...
def validate_sources():
    require(native.engine.file_digest(Path(previous.__file__)) == A173_SHA, "a173_helper_source_drift")
...
def system_instruction(core, constraint, selector):
    require(core in INSTRUCTION_CORES and constraint in CLOSING_CONSTRAINTS and selector in SELECTORS, "system_factor_levels")
    return CORE_TEXT[core].format(selector=selector) + " " + CLOSING_TEXT[constraint]


def compile_plan(config_sha256, protocol_sha256, tests_sha256):
    """Compile the fixed public crossover; never read parent experimental data."""
    sources = validate_sources()
    require(config_sha256 == CONFIG_SHA and tasks.is_hash(protocol_sha256) and tasks.is_hash(tests_sha256), "plan_bindings")
    for selector in SELECTORS:
        require(system_instruction("legacy", "legacy", selector) == tasks.system_instruction(selector)
                and system_instruction("a171_standard", "a171_standard", selector) == previous.STANDARD.format(selector=selector),
                "exact_diagonal_reconstruction")
    ...
    for world_index, (value_a, value_b, presentation) in enumerate(WORLDS):
        ...
        for core, constraint, selector in schedule:
            system = system_instruction(core, constraint, selector)
            trial = {"world_index": world_index, "world_id": f"w{world_index:02d}", "presentation_order": presentation,
                     "instruction_core": core, "closing_constraint": constraint, "selector": selector,
                     "selected_answer": mapping[selector], "unselected_answer": mapping["B" if selector == "A" else "A"],
                     "sequence_index": len(trials),
                     "messages": [{"role": "system", "content": system}, {"role": "user", "content": user}]}
            trial["trial_id"] = object_sha({"schema_version": "a174-cell-identity-v1", "bindings": bindings, "trial": trial})[:24]
            trials.append(trial)
    require(len(trials) == 32 and len({t["trial_id"] for t in trials}) == 32, "fixed_matrix")
    return {"schema_version": "a174-plan-v1", "partition": "fresh_public_synthetic", "bindings": bindings,
            "trials": trials, "world_count": 4, "planned_cells": 32, "maximum_generation_calls": 32,
            ...
            "primary_contrast_directions": {axis: {"positive": levels[1], "negative": levels[0]} for axis, levels in zip(AXES, LEVELS)},
            "secondary_interaction": {"name": "core_by_constraint_interaction", "positive_cells": [["legacy", "legacy"],
                ["a171_standard", "a171_standard"]], "negative_cells": [["legacy", "a171_standard"], ["a171_standard", "legacy"]],
                "planned_quartets": 8, "cell_coefficient_magnitude": 0.125},
...
def _inputs(plan_path, plan_sha256, config_path, config_sha256, protocol_path, reference_script, tokenizer_loader):
    ...
    require(len({object_sha(p["prompt_token_ids"]) for p in prepared.values()}) == 32, "unique_native_cells")
...
def prepare_inputs(*, plan_path, plan_sha256, config_path, config_sha256, protocol_path, reference_script,
    ...
    return {"schema_version": "a174-native-freeze-v1", "plan_sha256": plan_sha256, "config_sha256": config_sha256,
            "protocol_sha256": plan["bindings"]["protocol_sha256"], "tests_sha256": plan["bindings"]["tests_sha256"],
            "source_hashes": sources, "reference_script_sha256": REFERENCE_SHA, "planned_cells": 32,
            "maximum_generation_calls": 32, "eos_token_ids": eos,
...
def _private_root(path):
    ...
    require(path.is_relative_to(PRIVATE_RUNS_ROOT) and path != PRIVATE_RUNS_ROOT, "a174_private_run_root")
...
def _attempt(header, trial, prepared):
    return {"schema_version": "a174-attempt-v1", "run_sha256": object_sha(header), "trial_sha256": object_sha(trial),
...
def _startup_attempt(pid, plan_sha256, config_sha256):
    return {"schema_version": "a174-startup-attempt-v1", "pid": pid, "plan_sha256": plan_sha256,
            "config_sha256": config_sha256, "source_sha256": native.engine.file_digest(Path(__file__))}


def _interruption(exc):
    return {"signal_number": exc.signum if isinstance(exc, A174Interrupted) else None,
            "signal_name": exc.signal_name if isinstance(exc, A174Interrupted) else None,
            "pid": exc.pid if isinstance(exc, A174Interrupted) else os.getpid(), "exception_type": type(exc).__name__}
...
def _validate_result(result, attempt, trial, tokenizer, eos):
    require(type(result) is dict and set(result) == {"schema_version", "attempt_sha256", "status", "generated_token_ids",
            "score", "error", "elapsed_seconds"} and result["schema_version"] == "a174-result-v1"
...
def _record(trial, result, attempted):
    metadata = {key: trial[key] for key in ("trial_id", "sequence_index", "world_index", "world_id", "presentation_order",
                                           "instruction_core", "closing_constraint", "selector")}
    ...
    return {"schema_version": "a174-cell-v1", **metadata, **score}


def _binary_bound(records, field, axis):
    """Equal-weight 16-pair main effect, with sharp signed missingness bounds."""
    require(axis in AXES and len(records) == 32, "contrast_axis_and_cohort")
    ...
    return {"point": fixed if not unresolved else None,
            ...
            "planned_outcomes": 32, "resolved_outcomes": len(known), "planned_pairs": 16,
...
def _interaction_bound(records):
    """Secondary difference of differences: eight equally weighted quartets."""
    require(len(records) == 32, "interaction_cohort")
    quartets, terms = {}, []
    expected = set(product(INSTRUCTION_CORES, CLOSING_CONSTRAINTS))
    for row in records:
        cell = (row["instruction_core"], row["closing_constraint"])
    ...
    return {"point": fixed if not unresolved else None,
            "lower": math.fsum([fixed, *(min(0., coefficient) for coefficient in unresolved)]),
            "upper": math.fsum([fixed, *(max(0., coefficient) for coefficient in unresolved)]),
            "planned_outcomes": 32, "resolved_outcomes": len(known), "planned_quartets": 8,
...
def _aggregate(plan, records, results, attempted):
    ...
    return {"schema_version": "a174-analysis-v1", "status": "complete" if completed == 32 else "incomplete",
            "planned_worlds": 4, "planned_cells": 32, "maximum_generation_calls": 32,
            "coverage": {"attempted": len(attempted), "completed": completed, "infrastructure_failed": len(results) - completed,
                         "interrupted": len(attempted) - len(results), "unattempted": 32 - len(attempted),
                         "missing": 32 - len(results)},
            "overall": _counts(records),
            "by_condition": {core: {constraint: _counts([r for r in records if r["instruction_core"] == core
                and r["closing_constraint"] == constraint]) for constraint in CLOSING_CONSTRAINTS} for core in INSTRUCTION_CORES},
            "contrasts": {"strict_accuracy": _contrasts(records)},
            "secondary_contrasts": {"strict_accuracy": {"core_by_constraint_interaction": _interaction_bound(records)}},
...
def _fixed_header(plan, config, sources, eos, prepared, plan_sha256, config_sha256, reference_script):
    return {"schema_version": "a174-run-v1", "plan_sha256": plan_sha256, "config_sha256": config_sha256,
            ...
            "planned_cells": 32, "maximum_generation_calls": 32, "maximum_seconds": MAX_SECONDS,
...
def _replay(root, plan, config, sources, tokenizer, eos, prepared, plan_sha256, config_sha256, reference_script):
    ...
    require(_same(state._read(root / "one-shot.json"), {"schema_version": "a174-one-shot-v1", "pid": header["pid"],
    ...
    if (root / "execution-finished.json").exists():
        ...
        require(type(finished) is dict and set(finished) == {"schema_version", "status", "elapsed_seconds", "run_sha256", "interruption"}
                and finished["schema_version"] == "a174-execution-finished-v1" and finished["run_sha256"] == object_sha(header)
        ...
        if finished["status"] in ("interrupted", "deadline"):
            require(type(interruption) is dict and set(interruption) == {"signal_number", "signal_name", "pid", "exception_type"}
                    ...
                    ("A174Interrupted", "A174Deadline", "KeyboardInterrupt"), "interruption_receipt")
            allowed = {"A174Interrupted": (signal.SIGTERM, signal.SIGINT, signal.SIGHUP),
                       "A174Deadline": (signal.SIGALRM,), "KeyboardInterrupt": (None,)}
            ...
            require((number is None or type(number) is int) and number in allowed[interruption["exception_type"]] and interruption["signal_name"] ==
                    ...
                    and (finished["status"] == "deadline") == (interruption["exception_type"] == "A174Deadline"), "actual_signal_receipt")
        else:
...
def _interrupt(signum, _frame):
    raise A174Interrupted(signum)


def _deadline(signum, _frame):
    raise A174Deadline(signum)
...
def run_plan(*, plan_path, plan_sha256, config_path, config_sha256, protocol_path, reference_script,
    ...
    """One fresh model, all 32 scheduled cells, no outcome gate or retry."""
    ...
    with (root / ".lock").open("a") as lock:
        ...
        with contextlib.ExitStack() as stack:
            try:
                ...
                native.engine.write_private(root / "one-shot.json", {"schema_version": "a174-one-shot-v1", "pid": os.getpid(), "plan_sha256": plan_sha256})
                ...
                for trial in plan["trials"]:
                    ...
                    tick = time.monotonic()
                    result = {"schema_version": "a174-result-v1", "attempt_sha256": object_sha(attempt), "status": "infrastructure_failed",
                    ...
                    try:
                        ...
                        result.update(status="infrastructure_failed", generated_token_ids=None, score=None,
                                      error=native.engine.safe_error(exc, "a174_generation_failed"))
                    result["elapsed_seconds"] = time.monotonic() - tick
                    _validate_result(result, attempt, trial, tokenizer, eos)
                    native.engine.write_private(folder / "result.json", result)
                status = "finished_schedule"
            except (A174Interrupted, KeyboardInterrupt) as exc:
                error = exc
                status = "deadline" if isinstance(exc, A174Deadline) else "interrupted"
                ...
                runtime = None
                if header is None:
                    native.engine.write_private(root / "startup-failure.json", {"schema_version": "a174-startup-failure-v1",
                        ...
                        "error": native.engine.safe_error(error, "a174_startup_failed") if error is not None else None})
                else:
                    if error is not None:
                        native.engine.write_private(root / "error.json", native.engine.safe_error(error, "a174_execution_failed"))
                    native.engine.write_private(root / "execution-finished.json", {"schema_version": "a174-execution-finished-v1", "status": status,
                        "elapsed_seconds": time.monotonic() - started, "run_sha256": object_sha(header), "interruption": interruption})
        if header is None:
            if error is not None:
                raise error
            raise RuntimeError("a174_startup_failed_without_exception")
...
def main(argv=None):
    ...
    except (Exception, A174Interrupted, KeyboardInterrupt) as exc:
        print(json.dumps({"status": "a174_failed", "error_type": type(exc).__name__}))
        return 1

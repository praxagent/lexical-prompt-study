"""A188 fixed generation-only arithmetic task-load acquisition.

Inject a qualified CPU BF16 model/tokenizer; no asset loading or work on import.
...
def require(ok, reason):
    if not ok:
        raise ValueError("a188_acquisition_" + reason)
...
def source_bindings():
    ...
    return {
        ...
        "a184": A184_SHA,
        "a185": A185_SHA,
        "bf16_forward": e.sha256(Path(primitive.__file__).read_bytes()),
...
def compile_plan(
    *, provenance, geometry, eos_token_ids, runtime_binding_sha256, protocol_sha256, tests_sha256
    ...
    return {
        ...
        "study_id": "a188_bf16_arithmetic_task_load",
        "rows": rows,
